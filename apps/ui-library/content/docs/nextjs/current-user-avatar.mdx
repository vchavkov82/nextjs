...
description: BA Auth-aware avatar
---
...
The `CurrentUserAvatar` component connects to BA Auth to fetch the user data and show an avatar. It uses the `user_metadata`
property which gets populated automatically by BA Auth if the user logged in via a provider. If the user doesn't have a profile image, it renders their initials. If the user is logged out, it renders a `?` as a fallback, which you can change.
...
- [Auth users](https://www.assistance.bg/docs/guides/auth/users)
