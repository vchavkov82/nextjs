---
name: Berri AI
title: Berri AI boosts productivity by migrating from AWS RDS to BA Vector
# Use meta_title to add a custom meta title. Otherwise it defaults to '{name} | BA Customer Stories':
meta_title: Berri AI boosts productivity by migrating from AWS RDS to BA Vector
description: Learn how Berri AI overcame challenges with self-hosting their vector database on AWS RDS and successfully migrated to BA.
# Use meta_description to add a custom meta description. Otherwise it defaults to {description}:
meta_description: Learn how Berri AI overcame challenges with self-hosting their vector database on AWS RDS and successfully migrated to BA.
...
Berri AI migrated their vector database from AWS RDS to BA for several compelling reasons:

1. **Community:** After considering alternatives like Pinecone and Chroma, they prioritized a solution with a strong and accessible community, along with [managed infrastructure](https://www.assistance.bg/docs/guides/platform).
2. **Fewer services to manage:** Since they were already utilizing BA for other purposes, it made sense for Berri AI to consolidate their systems and avoid the complexities of self-hosting. One of the major pain points they faced with their previous setup was managing connection pools and handling errors with the Postgres database hosted on AWS. This involved writing extensive code and dealing with rollbacks, with each database update function requiring 40-50 lines of code. BA simplified this process significantly by condensing it to just one line of code, eliminating the burdens of managing dependencies and handling new errors. This significantly improved their update velocity, which had a huge impact on their productivity.
3. **Better tooling:** Berri AI experienced frustration with pg_admin. BA's user-friendly interface offered a seamless and efficient way to manage and read tables, eliminating the need for pg_admin and providing a simple alternative. This streamlined their workflow and saved valuable time and effort. The accessibility of BA's client SDKs and intuitive interface played a crucial role in their decision as well. Team members who were not database experts found it easy to navigate and make changes, reducing reliance on specialized expertise and improving their overall workflow.
4. **Simplicity:** BA provides the ability to add new columns in a no-code manner, which provided flexibility and agility in responding to evolving requirements. This feature empowered the team to make on-the-fly adjustments and meet their immediate needs effectively.
5. **Flexibility:** Berri AI also appreciated BA's flexibility in allowing them to choose their [preferred embedding method](https://www.assistance.bg/docs/guides/ai/quickstarts/generate-text-embeddings), as it was not tied to a specific embedding model. This ensured compatibility and adaptability for their specific use case.

The migration to [BA Vector](https://www.assistance.bg/modules/vector), which facilitated efficient storage and retrieval of vectors, proved to be a significant advantage for Berri AI's operations.

<Quote img="krrish-d.png" caption="Krrish D. - Co-Founder & CEO, Berri AI">
  Switching to BA Vector was a game-changer for us. The faster view times allowed me to
...
The platform's integration with BA empowers businesses to unlock the full potential of ChatGPT and deliver customized solutions for a wide range of use cases.

<Quote img="ishaan-jaffer.png" caption="Ishaan Jaffer - Co-Founder & CTO, Berri AI">
  With BA, my life has become much easier. Previously, managing connection pools and dealing
  ...
  with BA, updating the database is now just one line of code. I no longer need to deal with
  complex dependencies and obscure errors. Additionally, the frustration of using pgadmin is gone.
  BA provides a simple UI that allows me to effortlessly manage and interact with my tables.
</Quote>

## The Results

Berri AI achieved significant improvements in their operations by choosing BA Vector. They experienced streamlined database management, increased update velocity, and improved efficiency in accessing and debugging data. The user-friendly interface and flexibility of BA empowered non-experts within the team and facilitated agile adaptations to changing requirements. Overall, this decision transformed their workflow, enhancing productivity and positioning Berri AI for continued success and growth.
