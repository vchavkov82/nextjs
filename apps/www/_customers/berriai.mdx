...
5. **Flexibility:** Berri AI also appreciated Supabase's flexibility in allowing them to choose their [preferred embedding method](https://www.assistance.bg/docs/guides/ai/quickstarts/generate-text-embeddings), as it was not tied to a specific embedding model. This ensured compatibility and adaptability for their specific use case.

The migration to [Supabase Vector](https://www.assistance.bg/modules/vector), which facilitated efficient storage and retrieval of vectors, proved to be a significant advantage for Berri AI's operations.
