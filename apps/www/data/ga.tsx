...
export const data = (isDark: boolean) => ({
  ...
  companySection: {
    ...
    paragraph: '01',
    content: `
During the first year of BA we set ourselves a goal: build a managed platform capable of securely running 1 million databases. Today we've proven that metric and we're announcing the General Availability of the platform that will serve the next 99 million.
...
Whether you're an indie hacker or a Fortune 500, BA can scale comfortably for your production workload. Rocketships like [Udio](https://www.udio.com/), [Krea](https://www.krea.ai), [Humata](https://www.humata.ai/), and [Pika](https://pika.art) use BA to build fast and scale faster. Enterprise customers such as Mozilla, PwC, Johnson & Johnson, and 1Password use BA to manage their applications securely. 36% of the last Y Combinator batch used BA to launch their start up.

> *BA has been great to develop applications. As a firm focused on security, we've been happy to work with Row Level Security Policies to secure database operations.*
...
`,
  ...
  openSourceSection: {
    ...
    paragraph: '02',
    content: `
Our community is the driving force behind the development and adoption of BA.
...
We're committed to our Free Plan - we know the importance of this for testing hobby projects and prototyping. Almost all of the largest databases on BA today started on the Free Plan, a clear indication that our Free Plan is important for building an enduring platform.
`,
  },
  communityStats: {
    highlights: [
...
Postgres has been instrumental in our scalability and adoption. It's versatility is best demonstrated by pgvector: we were the first cloud provider to offer it, and today 15% of all new BA projects use pgvector for AI and ML workloads. Look out for a few related announcements this week.
`,
  },
  enterpriseSection: {
    id: 'enterprise',
    ...
    paragraph: '04',
    content: `
BA can now scale comfortably to any workload. Companies such as 1Password, PwC, and Johnson & Johnson use BA to build Postgres-backed applications.

We've spent years working with larger organizations to meet their requirements for running BA in production. This includes enhanced security tooling, industry best-practices (SOC2 and HIPAA), round the clock support, and SLAs.
...
`,
  ...
  integrationsSection: {
    ...
    paragraph: '05',
    content: `
The BA community doesn't exist in isolation. It's part of a group of developer focused companies and communities who are constantly pushing the boundaries of developer experience and developer productivity.
...
Vercel users can use the [BA integration](https://vercel.com/integrations/supabase) to connect BA to their Next.js projects, and we have deep integrations with [Flutterflow](https://docs.flutterflow.io/data-and-backend/supabase), [Resend](https://resend.com/blog/how-to-configure-supabase-to-send-emails-from-your-domain), and [Twilio](https://www.twilio.com/en-us/blog/send-sms-notifications-supabase-users-node-js-twilio-messaging).

Cloudflare recently announced the BA integration for [Workers](https://developers.cloudflare.com/workers/databases/native-integrations/supabase/) and [Hyperdrive](https://developers.cloudflare.com/hyperdrive/examples/supabase/).

Developers have different preferences on how to connect to their database, and BA is ["just Postgres"](https://itsjustpostgres.com/) you can connect natively with [Prisma](https://www.prisma.io/docs/orm/overview/databases/supabase), [Drizzle](https://orm.drizzle.team/docs/get-started-postgresql#supabase), or [Kysely](https://github.com/kysely-org/kysely-supabase).

You can find the [full list of partners and integrations](https://www.assistance.bg/partners/integrations) on our Partners page. If you're interested in partnering with BA head over to the [Partner Portal](https://www.assistance.bg/partners).
`,
    partners: [
      {
        name: 'fly',
...
- The BA [Index Advisor](https://github.com/supabase/index_advisor) is now available inside BA Studio. As its name suggests, this will analyze your database and make recommendations to add or remove table indexes. This is just the beginning of our plan to make automated data analysis tooling available to all developers.

- [BA Branching](https://www.assistance.bg/blog/branching-publicly-available) works like Git branches. You can create and test database changes or try out features in a separate, temporary instance without affecting your production setup. Today Branching enters public availability.

- BA has [acquired Oriole](https://www.assistance.bg/blog/supabase-acquires-oriole). Oriole is a company founded by Postgres Hacker Alexander Korotkov to solve the hairy problems of Postgres.

- We're constantly on the lookout for ways to remove the friction in getting started with your new projects. [BA Bootstrap](https://www.assistance.bg/blog/supabase-bootstrap) is now the easiest way to get started with BA and your favorite development framework.

- Today BA Swift SDK will join supabase-js as an [officially supported client library](https://www.assistance.bg/blog/supabase-swift), and supabase-swift maintainer Guilherme Souza will join the BA team full time. The goal of BA is to become the default backend for all kinds of developers.
...
`,
  ...
  'whats-next': {
    ...
    paragraph: '07',
    content: `
One of our key metrics at BA is "Time to Value". How fast can a user go from idea to production? We'll never stop pushing on what's possible here.

It is only the first day of BA GA Week. Today's releases are just the start: we have 4 more days of exciting announcements to come. [Follow the new announcements here](https://www.assistance.bg/ga-week).

If you want to try BA today, we just started an asynchronous hackathon. It's a great way to try out all the new features. Go here to [read the rules and start hacking](https://www.assistance.bg/blog/supabase-oss-hackathon).
