...
Since launching our [Vector Toolkit](https://www.assistance.bg/modules/vector) a few months ago, the number of AI applications on Supabase has grown - a lot. Hundreds of new databases every week are using [pgvector](https://github.com/pgvector/pgvector).
...
Supabase also provides [Database Webhooks](https://www.assistance.bg/docs/guides/database/webhooks) which can trigger an Edge Function any time a row is inserted. This means you can upload plain text to your database and use a background job to convert the text to an embedding.
...
Supabase is mainly used to store embeddings, so that’s where we’re starting. Over time we’ll add more Hugging Face support - even beyond embeddings. To help you identify which Hugging Face model to use, we ran a detailed analysis and found that embeddings with [fewer dimensions are better](https://www.assistance.bg/blog/fewer-dimensions-are-better-pgvector) within pgvector. Fewer dimensions have several advantages:
...
- [Day 2: Supabase Local Dev: migrations, branching, and observability](https://www.assistance.bg/blog/supabase-local-dev)
