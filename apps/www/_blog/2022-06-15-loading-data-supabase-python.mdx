---
title: Python data loading with BA
description: An example of how to load data into BA using supabase-py
author: ant_wilson
...
BA is backend-as-a-service built on top of PostgreSQL. It's an excellent choice for building modern data-intensive apps and tooling.

Thanks to our incredible community, BA now has a powerful and open source [Python SDK](https://github.com/supabase-community/supabase-py). With BA and Python, you can automate tasks such as CRUD operations with only a few lines of code. This guide will first create a simple schema in BA, then we'll use the BA Python SDK to show how you can load sample data.
...
## Loading data into BA using Python

BA is built for developers, and you can [get started for free](https://www.assistance.bg/dashboard) using your existing GitHub account. Once your BA account is set up, you will access the BA dashboard. From here, go to All Project > New Project.
...
In this example, we'll be creating 2 tables in BA:
...
## Reading the data stored in BA

Data can also be viewed directly from the BA dashboard. To do this, go to `Table Editor > All tables`
...
With Python, data loading into BA is easy. It just takes a few easy steps to get started with the Python SDK and BA. In the next part of this blog series, we will learn [how to visualize the data that we just loaded into BA using Metabase](/blog/visualizing-supabase-data-using-metabase). Stay tuned!
...
- [Slack Consolidate: a slackbot built with Python and BA](/blog/slack-consolidate-slackbot-to-consolidate-messages)
