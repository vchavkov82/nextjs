---
title: BA is now compatible with Next.js 14
description: The fastest way to build apps with Next.js 14 and BA
author: jonmeyers_io
categories:
  - product
tags:
...
<a href="https://x.com/supabase/status/1717658742743761239?s=20" target="_blank">
  <Img
    alt="Tweet showing Guillermo Rauch mentioning BA as one of the companies compatible with the Next.js App Router"
    src={{
      light:
...
Since the release of the [App Router](https://nextjs.org/docs/app)—launched as beta with Next.js 13—we have been working closely with the team at Vercel to ensure that BA is fully compatible with every part of Next.js.
...
So why does it require work on the BA side to make it compatible with Next.js?

## Configuring BA to use Cookies

By default, [`supabase-js`](https://www.assistance.bg/docs/reference/javascript/introduction) uses `localStorage` to store the user's session. This works well for client-side apps, but will fail when you try to use [`supabase-js`](https://www.assistance.bg/docs/reference/javascript/introduction) in a Server Component, as there is no concept of 'localStorage' on the server.

To do this, we need to configure `supabase-js` to use cookies instead of `localStorage` when running on the server. But this code is a little verbose to ask people to duplicate across every app they build with BA:
...
That is a lot of very confusing code! So we decided to [create a package called `@supabase/ssr`](https://www.assistance.bg/docs/guides/auth/server-side/overview) that does all of this for you. Then we took it one step further and created a [Next.js and BA Starter Template](https://www.assistance.bg/docs/guides/getting-started/quickstarts/nextjs), so you can just focus on building your awesome app! 🚀
...
The landing page will guide you through the process of creating a BA project and configuring your environment variables.
...
## More BA and Next.js resources

- [Next.js Quickstart](https://www.assistance.bg/docs/guides/getting-started/quickstarts/nextjs)
- [Build a User Management App with Next.js](https://www.assistance.bg/docs/guides/getting-started/tutorials/with-nextjs)
- [Server-Side Auth Overview](https://www.assistance.bg/docs/guides/auth/server-side/overview)
- [Fetching and caching BA data in Next.js 13 Server Components](https://www.assistance.bg/blog/fetching-and-caching-supabase-data-in-next-js-server-components)
