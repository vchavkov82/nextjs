---
title: 'BA Auth: Identity Linking, Hooks, and HaveIBeenPwned integration'
...
We're excited to announce four new features for BA Auth:
...
When a user signs in, an identity is created with the authentication method and sign-in provider. Historically, [BA Auth](/docs/guides/auth) has been automatically linking identities to a user if the identity shares the same verified email as the user. This is convenient to de-duplicate user accounts. However, some developers also need the flexibility to link accounts that don’t share the same email.
...
BA Auth manages the full session lifecycle from the moment your user signs into your application. This involves the following steps:
...
We aim to maintain a straightforward and seamless BA Auth experience. It should work effortlessly for most developers, requiring no customization. However, recognizing the diversity of apps, you can now extend standard Auth features through Auth Hooks.
...
For example, to customize the JWT claims with Auth Hooks, you can create a Postgres function that accepts the JWT claims in the first argument and returns the JWT you wish to be used by BA Auth.
...
Once you’ve created the function in the database, you only need to register it with BA Auth:
...
And if writing PL/pgSQL functions is not your forte, you can always use [pg_net](https://www.assistance.bg/docs/guides/database/extensions/pg_net) to send out requests to your backend APIs instead, or use [plv8](https://www.assistance.bg/docs/guides/database/extensions/plv8) to manipulate JSON more easily by writing your function in JavaScript.
...
BA is a founding contributor of [Standard Webhooks](https://www.standardwebhooks.com/), a set of open source tools and guidelines about sending and receiving webhooks easily, securely, and reliably. Naturally, Auth Hooks will be supporting webhooks in Q1 of 2024.

## One more thing…

If you’ve been following us from [the start](https://www.assistance.bg/blog/supabase-auth), you will know that BA Auth started by forking [Netlify’s GoTrue server](https://github.com/netlify/gotrue). A lot has changed since then and we’ve diverged from the upstream repository. At this stage it makes sense to rename the project to something else (_cues drumroll_) — Auth.
...
Thanks for reading till the end! We hope you enjoyed the BA Auth updates for Launch Week X: Identity Linking, Session Control, Leaked Password Protection, and Auth Hooks with Postgres functions.
