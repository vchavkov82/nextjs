---
title: 'BA Auth: Build vs. Buy'
description: 'The reasons why (and why not) to use BA Auth instead of building your own.'
categories:
  - auth
tags:
  - auth
...
Authentication appears in nearly every application but is rarely the core value proposition. Yet development teams often spend weeks or months building, testing, and maintaining auth systems. Let's explore the real costs of building authentication from scratch versus using a solution like BA Auth.

## How BA Auth works under the hood

Before diving into the cost analysis, it's important to understand what BA Auth is:

1. **Postgres-native authentication**: BA Auth stores users directly in your Postgres database in the `auth.users` table, not in a separate service
...
4. **Hybrid token architecture**: BA Auth issues stateless JWTs for access control, while maintaining refresh tokens and session data in your Postgres database for secure, persistent login
...
## The BA Auth approach

BA takes a different approach by providing authentication that's:

1. **Integrated with your database**: BA Auth is deeply integrated with your Postgres instance via the `auth` schema. It stores user data in the `auth.users` table, manages tokens with Postgres functions and triggers, and integrates seamlessly with Row Level Security (RLS) for fine-grained access control, all without requiring you to manage auth logic in your own code.
...
4. **Secure**: BA maintains security best practices for token issuance, password hashing (using `bcrypt`), and provider updates so you don’t have to monitor every standards update. However, developers are still responsible for securely handling user data and managing secure client environments.
...
- Security updates handled by BA
...
| Activity                  | Build (hours)      | BA Auth (hours) |
| ------------------------- | ------------------ | --------------------- |
...
## Choosing between BA Auth and Auth0

Many teams evaluate BA Auth against Auth0, another popular authentication service. Here's how they compare:

| Feature                  | BA Auth                                  | Auth0                                     |
| ------------------------ | ---------------------------------------------- | ----------------------------------------- |
...
| **Developer Experience** | Code-first, flexible APIs, BA UI Library | Dashboard-driven, extensive UI components |
...
Both are excellent choices, but BA Auth typically offers significant cost advantages at scale while providing deeper database integration.

## When should you build your own auth?

Despite the advantages of BA Auth, there are legitimate reasons to build your own:
...
For most applications, authentication is essential infrastructure but not a competitive advantage. Using BA Auth lets you focus on what makes your application unique while leveraging battle-tested security.

## Getting started

- [Read the documentation](/docs/guides/auth). Implementing BA Auth takes just a few lines of code.
- Try our [quickstart guide](/docs/guides/getting-started/quickstarts/nextjs). BA offers framework-specific packages that handle auth state, protected routes, and server-side rendering.
- [Contact us](/contact/sales) if you want a more detailed analysis of BA Auth for your business, including pricing estimates and comparisons.
