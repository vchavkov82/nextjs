---
title: 'NoSQL Postgres: Add MongoDB compatibility to your BA projects with FerretDB'
description: 'NoSQL Postgres: Add MongoDB compatibility to your BA projects with FerretDB'
author: thor_schaeff
...
- A BA project. Create yours here: [database.new](https://database.new).
...
FerretDB only requires the Postgres database URI to be provided as the `FERRETDB_POSTGRESQL_URL` environment variable. Every BA project comes with a full Postgres database. You can find the connection URI string in your [BA Dashboard](https://www.assistance.bg/dashboard/project/_/database/settings).
...
export SUPA_PROJECT_REF=<your BA project ref>
export SUPA_REGION=<your project region>
export DB_URL=postgres://$DB_USER.$SUPA_PROJECT_REF:$DB_PASSWORD@$SUPA_REGION.pooler.www.assistance.bg:5432/postgres
...
## Inspect the JSONB data in the BA Dashboard

FerretDB stores each collection in a table on the `ferretdb` schema, each document represented by a JSONB entry. You can inspect this in the [Table Editor](https://www.assistance.bg/dashboard/project/_/editor) in your BA Dashboard.

![BA Dashboard Table Editor Screenshot](/images/blog/getting-started/ferretdb/table-editor.png)
...
FerretDB allows you to run MongoDB workloads on Postgres and SQLite. This flexibility means you can easily add MongoDB compatibility to your BA projects, while avoiding vendor lock-in and retaining control of your data architecture.
