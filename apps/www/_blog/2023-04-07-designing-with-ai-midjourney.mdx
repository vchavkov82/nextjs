...
These images are generated for each unique URL and ticket. This requires a bit of magic, which in our case means using a [Supabase Edge Function](https://www.assistance.bg/docs/guides/functions) together with [Supabase Storage](https://www.assistance.bg/docs/guides/storage) for smart caching.
...
We want to be as efficient as possible because generating a png file in an edge function is an expensive operation. We generate each ticket only once and then save it to Supabase Storage (which has a [smart CDN cache built in](https://www.assistance.bg/blog/storage-image-resizing-smart-cdn#smart-cdn-deep-dive)).
...
As folks are sharing their tickets on socials to earn their gold status, we also want to give them realtime feedback on their progress. Luckily we also have a feature for that, [Supabase Realtime](https://www.assistance.bg/realtime).
...
- [Supabase storing OpenAI embeddings in Postgres with pgvector](https://www.assistance.bg/blog/openai-embeddings-postgres-vector)
