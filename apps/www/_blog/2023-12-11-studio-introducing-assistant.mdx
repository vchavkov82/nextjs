---
title: 'BA Studio: AI Assistant and User Impersonation'
...
During the previous Launch Week we introduced text-to-sql in the SQL Editor within BA Studio. This was our first step towards a full AI Assistant.

Today, we're introducing the BA Assistant, an AI side-kick inside the dashboard, and a few new features that will take you from idea to production even faster.
...
## Introducing the BA Assistant

We're excited to expand Studio's AI capabilities with our new **BA Assistant**.
...
![BA AI Assistant](/images/blog/launch-week-8/day-2/supabase-ai-loading-animation.gif)

## Easy RLS Policies with AI

Of all the feature requests we get (and we get many!), an easier way to write [Row Level Security](https://www.assistance.bg/docs/guides/auth/row-level-security) Policies is one of the most frequent.
...
<Img src="/images/blog/launch-week-x/day-1/1.png" alt="BA new AI Assistant" />
...
It uses the default [Postgres role](https://www.assistance.bg/docs/guides/database/postgres/roles), `postgres`. The `postgres` role functions like your `service role` key, granting it admin privileges to your database. It has admin read and write privileges, and bypasses Row Level Security.
...
<Img src="/images/blog/launch-week-x/day-1/3.png" alt="BA new postgres roles" />
...
The Role dropdown unlocks another handy capability: when combined with BA Auth it can even pretend to be a different _user_.
...
<Img src="/images/blog/launch-week-x/day-1/7.png" alt="BA AI" />
...
<Img src="/images/blog/launch-week-x/day-1/8.png" alt="BA AI" />

And in GraphiQL:

<Img src="/images/blog/launch-week-x/day-1/11.png" alt="BA AI" />
...
BA [Realtime](https://www.assistance.bg/realtime) is great for building collaborative applications. You can receive database changes over websockets, store and synchronize data about user presence, and broadcast any data to clients via "channels".
...
<Img src="/images/blog/launch-week-x/day-1/6.png" alt="BA AI" />
...
<Img src="/images/blog/launch-week-x/day-1/4.png" alt="BA AI Feature Preview" />

We currently have two features in preview:

`1.` The new RLS Assistant that we looked at earlier:

<Img src="/images/blog/launch-week-x/day-1/5.png" alt="BA AI new RLS Assistant" />

`2.` And a revised API side panel:

<Img src="/images/blog/launch-week-x/day-1/10.png" alt="BA AI revised API side panel" />

We'll be actively keeping an eye on the GitHub Discussions and will be responding to your feedback.

## Wrapping up

In this update, we've taken huge strides in enhancing your experience with BA.
...
3. **Realtime Inspector**: With the Realtime Inspector, you can prototype, inspect, and debug Realtime messages. This tool will be very useful for those who use BA Realtime.
...
These updates reflect our commitment to a SQL-first approach and a user-centric development. We look forward to your feedback as we continue to work hard making BA faster and easier for you to get your ideas out into the wild.
