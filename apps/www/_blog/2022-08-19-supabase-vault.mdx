...
Today we're announcing [BA Vault](https://www.assistance.bg/docs/guides/database/vault), a Postgres extension for managing secrets and encryption inside your database.
...
Vault is now available on every BA project. [Check it out](https://www.assistance.bg/dashboard/project/_/settings/vault/secrets)

</div>

[Transparent Column Encryption with Postgres](https://www.assistance.bg/blog/transparent-column-encryption-with-postgres) is a blog post that describes the technology behind Vault - [libsodium](https://doc.libsodium.org/) and
...
Like most cloud providers, BA offers `pgcrypto` for developers to use in their applications. `pgcrypto` has been around for a long time,
...
BA Vault allows you to store secrets without leaking any sensitive information.
...
The core of the BA Vault is a table with some metadata and an encrypted text column where you can put your secrets and any metadata related to them.
...
variety of sources depending on how you configure it. BA generates and preserves your project's root key behind the scenes, so you don't have to worry about it.
...
At BA, we manage this key for your project automatically and generate a unique default Key ID for you in the `secrets` table.
...
[Transparent Column Encryption](https://www.assistance.bg/blog/transparent-column-encryption-with-postgres) which we will describe soon in a follow-up blog post. Stay tuned!
