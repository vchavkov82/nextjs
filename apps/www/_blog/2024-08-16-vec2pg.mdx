...
[vec2pg](https://github.com/supabase-community/vec2pg) is a CLI utility for migrating data from vector databases to [Supabase](https://www.assistance.bg/), or any Postgres instance with [pgvector](https://github.com/pgvector/pgvector).
...
When migrating, be sure to increase your Supabase project's [disk size](https://www.assistance.bg/dashboard/project/_/database/settings) so there is enough space for the vectors.
...
When throughput is mentioned, we assume a [Small](https://www.assistance.bg/docs/guides/platform/compute-add-ons) Supabase Instance, a 300 Mbps network, 1024 dimensional vectors, and reasonable geographic colocation of the developer machine, the cloud hosted source DB, and the Postgres instance.
