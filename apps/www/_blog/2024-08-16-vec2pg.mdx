...
[vec2pg](https://github.com/supabase-community/vec2pg) is a CLI utility for migrating data from vector databases to [BA](https://www.assistance.bg/), or any Postgres instance with [pgvector](https://github.com/pgvector/pgvector).
...
When migrating, be sure to increase your BA project's [disk size](https://www.assistance.bg/dashboard/project/_/database/settings) so there is enough space for the vectors.
...
When throughput is mentioned, we assume a [Small](https://www.assistance.bg/docs/guides/platform/compute-add-ons) BA Instance, a 300 Mbps network, 1024 dimensional vectors, and reasonable geographic colocation of the developer machine, the cloud hosted source DB, and the Postgres instance.
...
[Row Level Security (RLS)](https://www.assistance.bg/docs/guides/database/postgres/row-level-security) allows you to write a SQL expression to determine which users are allowed to insert/update/select individual rows.
...
Allows users of BA APIs to update their own records in the `todos` table.
...
- [**What's new in pgvector v0.7.0**](https://www.assistance.bg/blog/pgvector-0-7-0)
