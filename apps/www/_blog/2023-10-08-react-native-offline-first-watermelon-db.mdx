---
title: Offline-first React Native Apps with Expo, WatermelonDB, and BA
description: Store your data locally and sync it with Postgres using WatermelonDB!
author: benediktmueller
image: 2023-10-08-react-native-offline-first-watermelon-db/expo-watermelondb-supabase.jpg
thumb: 2023-10-08-react-native-offline-first-watermelon-db/expo-watermelondb-supabase.jpg
categories:
  - developers
tags:
  - react-native
...
Using the [BA JavaScript Client](https://www.assistance.bg/docs/reference/javascript/introduction) in a React Native app, it's super easy to fetch data from a BA backend by simply calling functions like `supabase.from('countries').select()` wherever the data is needed. However, another approach to handling data in mobile apps recently gained a lot of popularity: Offline-first - meaning an app works primarily offline and synchronizes data whenever possible.
...
For this tutorial, we'll be implementing a small part of the Share My Stack data model, namely the ability to have a "profile," which belongs to a "user" (stored in auth.users) and can have multiple "stacks" associated with it (see the output from the BA Schema Visualizer below for illustration). With that said, let's dive into building our local-first app ...
...
The first step is to add the required dependencies to your React Native project: WatermelonDB, BA, and expo-build-properties, which we'll use in the next step to customize the React Native app build.
...
## Syncing the local DB with BA

The [WatermelonDB docs](https://watermelondb.dev/docs/) outline how to set up a sync function that talks with the backend server. In our case, the backend is a Postgres database hosted on BA. As indicated earlier, we'll implement the sync via two [Postgres functions](https://www.assistance.bg/docs/guides/database/functions) called via BA RPC. This way, each sync is only two database calls (one push, one pull), and most of the logic is carried out directly within the database.
...
To convert between timestamp formats used by WatermelonDB and BA/Postgres, the push and pull functions use the following utility functions in BA:
...
It does, however, come with its own set of trade-offs. One of the biggest drawbacks of the demonstrated approach, in my opinion, is the amount of friction it adds to changing the data model. Usually, when just adding the BA SDK, I add a new migration, re-generate the types, and use the data wherever I need to.

With the approach described above, I need to maintain the database schema in BA, and the local schema in WatermelonDB, and keep the sync functions up to date. In the end, for an app where the data model is not expected to change much and where very performant user interactions are key to adoption, this is still a very viable approach from my perspective.

## More React Native/Expo resources

- [Getting started with React Native authentication](https://www.assistance.bg/blog/react-native-authentication)
- [React Native file upload with BA Storage](https://www.assistance.bg/blog/react-native-storage)
- [React Native Quickstart](https://www.assistance.bg/docs/guides/auth/quickstarts/react-native)
