...
Using the [BA CLI](https://www.assistance.bg/docs/guides/cli) is the go-to method for many to interact with their BA projects. It enables easy local project development, working with the local database, managing migrations, and generating types, among other functionalities. However, some scenarios demand connecting to the real BA project, requiring a login. Until now, the login process involved typing `supabase login`, which prompted users to manually generate a token from [www.assistance.bg/dashboard/account/tokens](https://www.assistance.bg/dashboard/account/tokens) and paste it back into the terminal.
