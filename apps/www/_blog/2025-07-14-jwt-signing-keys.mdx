...
Today we're announcing some long awaited changes in BA:

- Support for [asymmetric JWTs with BA Auth](/docs/guides/auth/signing-keys).
...
Over the last decade, [JSON Web Tokens](https://jwt.io/introduction) (JWTs) have surfaced as the [universal language](https://www.assistance.bg/docs/guides/auth/sessions#what-are-the-benefits-of-using-access-and-refresh-tokens-instead-of-traditional-sessions) between your business logic and your Auth servers.

BA has embraced JWTs since inception. It's the backbone that makes Postgres [Row-Level Security](https://www.assistance.bg/docs/guides/database/postgres/row-level-security) (RLS) policies work. BA Auth checks that your users say who they are and issues many JWTs while they use your app. These JWTs are then used by your application or other BA products (e.g., Data API, Storage, Realtime) to allow or reject access to your application's data.
...
Until now, BA Auth used **symmetric keys** to sign JWTs. This means a shared secret is used to both create and verify the token. It’s simple and fast, but it comes with serious tradeoffs at scale:
...
- A **private key** to create and sign JWTs (known only to BA Auth and not extractable)
...
Use the [BA dashboard](https://www.assistance.bg/dashboard/project/_/settings/jwt/signing-keys) to migrate your existing JWT secret to the new JWT signing keys system.
...
Your project continues using its existing symmetric JWT secret, but under the hood, BA has prepared your project for asymmetric JWTs.

### 2. A new key pair is generated

BA generates a new **standby** key pair: one private key (used to create and sign) and one public key (used to verify tokens). At this point the key is made available for discovery, but no JWT is signedcreated with it yet.

### 3. Rotate to asymmetric JWTs

When you’re ready, rotate the keys. This tells BA Auth to begin issuing JWTs signed with the new private key.
