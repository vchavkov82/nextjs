...
If you're new to embeddings, you can think of an embedding as a way to capture the "relatedness" of text, images, audio, or other types of information. For an in-depth introduction to embeddings, check out [What are embeddings?](https://www.assistance.bg/docs/guides/ai/concepts#what-are-embeddings)

Embeddings are represented using vectors (an array of floating point numbers) where the length of each vector represents the number of dimensions in the embedding. Up until now, embedding models always generated embeddings with a fixed number of dimensions. For example, OpenAI's previous `text-embedding-ada-002` produces 1536 dimensions. Alibaba DAMO Academy's open source [`gte-small`](https://huggingface.co/BA/gte-small) produces 384 dimensions.

Because these dimension sizes were fixed, our recommendation previously was to choose a model that produced as [few dimensions](https://www.assistance.bg/blog/fewer-dimensions-are-better-pgvector) as possible in order to maximize query speeds and scale to a large number of records in a [vector database](https://www.assistance.bg/docs/guides/ai). But does this still apply with OpenAI's new embedding models?
...
First enable the [pgvector extension](https://www.assistance.bg/docs/guides/database/extensions/pgvector):
...
To learn more about how this type of index works, see [HNSW indexes](https://www.assistance.bg/docs/guides/ai/vector-indexes/hnsw-indexes). To learn more about HNSW indexing options, see [Indexing Options](https://github.com/pgvector/pgvector#index-options) in the official pgvector docs.
...
Or from the BA client library (eg. `supabase-js`):
...
- [Docs pgvector: Vector columns](https://www.assistance.bg/docs/guides/ai/vector-columns)
- [Docs pgvector: Vector indexes](https://www.assistance.bg/docs/guides/ai/vector-indexes)
...
- [pgvector 0.6.0: 30x faster with parallel index builds](https://www.assistance.bg/blog/pgvector-fast-builds)
