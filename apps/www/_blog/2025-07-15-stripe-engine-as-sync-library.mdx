...
Previously distributed only as a Docker image (`supabase/stripe-sync-engine`), you can now plug this into any backend project—whether you're using Node.js, running Express on a server, or even deploying on BA Edge Functions.
...
While BA offers a convenient [foreign data wrapper](https://www.assistance.bg/partners/integrations/supabase_wrapper_stripe) (FDW) for Stripe, sometimes you want your Stripe data _locally available_ in your Postgres database for:
...
## Use via BA Edge Function

To use the Stripe-Sync-Engine in an [Edge Function](https://www.assistance.bg/edge-functions), you first have to ensure that the schema and tables exist. While you can technically do this inside the Edge Function, it is recommended to run the schema migrations outside of that. You can do a one-off migration via
...
2. Set up a Stripe webhook with the newly deployed BA Edge Function url
