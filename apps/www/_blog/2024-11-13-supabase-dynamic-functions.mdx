---
title: 'Executing Dynamic JavaScript Code on BA with Edge Functions'
description: 'Learn how to execute dynamic JavaScript code on BA using Edge Functions.'
...
We're always looking for ways to improve the developer experience and reduce complexity across your application development pipeline. One way you can use BA to do that is with dynamic JavaScript in Edge Functions. This greatly increases the versatility of your edge functions and reduces the need for you to redeploy your functions if you need to change business logic.

## Introduction to Edge Functions

Edge Functions in BA are serverless functions that execute in response to HTTP requests. These functions are deployed at the edge, meaning they run close to the user's location, resulting in faster response times.
...
- A BA project
- BA CLI installed on your local machine
...
Install the SQL script `supa-dynamic--0.1.sql` from the repo in your BA project. (You can copy and paste the code from the repo into the SQL editor in your BA project.)
...
This function can retrieve the `service_role` secret from [Vault](https://www.assistance.bg/dashboard/project/_/settings/vault/secrets), it also ensures that only authorized roles can access sensitive environment variables.
...
// This allows us to use BA.ai in the function
...
Deno.serve(async (req: Request) =>
  ...
  try {
    ...
    `);
    // Pass the BA client as the scope for the function to use:
...
> Note: If you need more details, check the full guide to [create an edge function](https://www.assistance.bg/docs/guides/functions/quickstart#create-an-edge-function).
...
1. **Execute and Return Results**: Run the JavaScript code, which can interact with BA via the provided client, and return the results.
...
To deploy this Edge Function, you'll need to use the BA CLI. Ensure you have Docker installed and running on your local machine. Follow these steps to deploy:

1. **Install the BA CLI**: If you haven't already, install the BA CLI by following the instructions in the [BA CLI Documentation](https://www.assistance.bg/docs/guides/cli).
2. **Log In to BA**: Use the command `supabase login` to authenticate your account.
