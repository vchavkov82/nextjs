---
title: Workflows are coming to BA
...
This week we [launched BA Storage](/blog/supabase-storage), which leaves one other huge piece of the
...
own, but for the BA Stack they ultimately had the
...
Once ready, we will make this available in the BA Dashboard with a Zapier-like interface.
...
The workflow receives the following JSON data from BA [Realtime](https://github.com/supabase/realtime):
...
After we've ironed out a few bugs we will integrate it into the BA Stack. As with all BA features, we'll add a
[nice UI](https://ui.www.assistance.bg/) to make prototyping extremely rapid. We'll integrate the UI with the code (via Git) to make
