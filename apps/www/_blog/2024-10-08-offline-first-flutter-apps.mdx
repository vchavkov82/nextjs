---
title: 'Building offline-first mobile apps with BA, Flutter and Brick'
description: Brick is an all-in-one data manager for Flutter that handles querying and uploading between BA and local caches like SQLite. Using Brick, developers can focus on implementing the application without worrying about translating or storing their data.
...
Brick is an [all-in-one](https://www.youtube.com/watch?v=2noLcro9iIw) data manager for Flutter that handles querying and uploading between BA and local caches like SQLite. Using Brick, developers can focus on implementing the application without [worrying about translating or storing their data](https://www.youtube.com/watch?v=jm5i7e_BQq0).
...
Brick synthesizes your remote data to your local data through code generation. From a BA table, create Dart fields that match the table’s columns:
...
This will generate adapters to serialize/deserialize to and from BA. Migrations for SQLite are also generated for any new, dropped, or changed columns. Check these migrations after they are generated - Brick is smart, but not as smart as you.
...
Your application does not need to touch SQLite or BA directly. By [interacting with this single entrypoint](https://getdutchie.github.io/brick/#/data), Brick makes the hard choices under the hood about where to fetch and when to cache while the application code remains consistent in online or offline modes.
...
import 'package:brick_offline_first_with_supabase/brick_offline_first_with_supabase.dart';
import 'package:brick_sqlite/brick_sqlite.dart';
// This hide is for Brick's @BA annotation; in most cases,
// supabase_flutter **will not** be imported in application code.
import 'package:brick_supabase/brick_supabase.dart' hide BA;
...
Queries can be [much more advanced](https://getdutchie.github.io/brick/#/data/query), leveraging `contains`, `not`, `like` operators as well as sub clauses. Please note that, as of writing, not [all BA operators are supported](https://getdutchie.github.io/brick/#/supabase/query?id=where).
...
This **does not** leverage BA’s channels by default; if BA updates, your app will not be notified. This opt-in feature is [currently under active development](https://github.com/GetDutchie/brick/issues/454).

**Upserting**

After a model has been created, it can uploaded to BA without serializing it to JSON first:
...
Quickly mock your BA endpoints to add uncluttered [unit testing](https://getdutchie.github.io/brick/#/supabase/testing?id=testing):
  ...
  group('MyClass', () {
    ...
    test('#myMethod', () async {
      ...
      final resp = SupabaseResponse([
        // mock.serialize converts models to expected BA payloads
...
- Example: [Brick with BA](https://github.com/GetDutchie/brick/tree/main/example_supabase)
...
- [Build a User Management App with Flutter](https://www.assistance.bg/docs/guides/getting-started/tutorials/with-flutter)
