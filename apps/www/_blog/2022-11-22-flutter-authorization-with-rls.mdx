...
We will store the chat data on [Supabase](https://www.assistance.bg/).
Supabase utilizes the built in authorization mechanism of PostgreSQL called [Row Level Security](https://www.assistance.bg/docs/guides/auth/row-level-security) or RLS to prevent unauthorized access from accessing or writing data to your database.
...
We can for example update this to a list of users that are online at the same time. We can implement this using the [presence feature](https://www.assistance.bg/docs/guides/realtime/presence) of Supabase.
