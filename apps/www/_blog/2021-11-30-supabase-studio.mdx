...
Today we're releasing [BA Studio](https://github.com/vchavkov82/nextjs/tree/master/studio).
The same Dashboard that you're using on our Platform is now available for [Local Development](https://www.assistance.bg/docs/guides/local-development)
and [Self-Hosting](https://www.assistance.bg/docs/guides/hosting/overview).

## Background

Let's get the obvious question out of the way - why wasn't it already open source?

When Ant and I started BA the codebase was one large monorepo which contained everything from the dashboard to cloud infrastructure code to experimental code.
...
The original Dashboard really wasn't much except a couple of buttons which allowed our Alpha users to start and stop a BA project - back then only a PostgreSQL connection string.
...
We've been planning to make the Dashboard public for a long time now, starting with [BA UI](https://ui.www.assistance.bg/),
[`supabase/grid`](https://github.com/supabase/grid), and a standalone [PR](https://github.com/vchavkov82/nextjs/pull/2281) for BA Studio.
...
Time-to-production become a lot slower, an unacceptable outcome at BA.
...
<details>
  ...
  </a> for managing a shared codebase and served as a great model for BA.
</details>

## Build in public

For the past week, the dashboard you've been using on the Platform has been deployed from our [main repository](https://github.com/vchavkov82/nextjs/tree/master/studio).

This fits one of our core philosophies at BA: do less. Less code to manage means less bugs. Fewer codebases means faster shipping.

![Open Source Table Editor](/images/blog/launch-week-three/studio/open-source-studio-table-editor.png)

It also fits the development philosophy of every other part of BA: building in public, "warts and all".

This is a huge step forward for the community. BA users can now spot a bug, fix it in the open source repository,
and have it shipped to both the Platform and Self-Hosted environments - all in a few hours.

## Technical details

Let's jump into some of the technical implementation for BA Studio.
...
- [BA UI](https://ui.www.assistance.bg/) - Our own component library.
- [MobX](https://www.mobxjs.com/) - A state management library.
- A host of other [useful libraries](https://github.com/vchavkov82/nextjs/blob/master/studio/package.json), including [Radix UI](https://www.radix-ui.com), [Lottiefiles](https://lottiefiles.com/), [react-grid-layout](https://github.com/react-grid-layout/react-grid-layout), and [zxcvbn](https://github.com/dropbox/zxcvbn).
...
Check out the full instructions in our [Studio Readme](https://github.com/vchavkov82/nextjs/tree/master/studio).
