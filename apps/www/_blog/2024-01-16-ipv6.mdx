...
- Are you connected to a BA database from your local machine using the [direct connection](https://www.assistance.bg/docs/guides/database/connecting-to-postgres#direct-connections)? You need to use the [connection pooler](https://www.assistance.bg/docs/guides/database/connecting-to-postgres#connection-pooler) which will resolve as IPv4 instead (we will pay for IPv4 addresses on these).
...
A lot of developer tools simply aren't set up for IPv6 yet. We can use BA as an example - our data team needed to make the following changes to support IPv6 with their toolchain:
...
If you are a BA customer, we have 3 simple solutions:

1. Switch your “direct” database connection to our new Supavisor database proxy. You can find these details in the [project connect page](https://www.assistance.bg/dashboard/project/_?showConnect=true).
