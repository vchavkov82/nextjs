---
title: 'BA is now on Postgres 13.3'
description: From today, new BA projects will be on a version of BA Postgres that runs on Postgres 13.3.
...
From today, new BA projects will be on a version of [BA Postgres](https://github.com/supabase/postgres) that runs
...
BA Postgres will follow PostgreSQL. This makes it much easier to ascertain what version of PostgreSQL is installed. In the situation wherein
...
<small>
  *Disclaimer for self-hosting: This is not available for x86 instances. All instances on the
  BA platform have this enabled by default.*
</small>
...
Below is a comparison between the ARM versions of BA Postgres `0.15.0` and `13.3.0`. Both are using `m6gd.8xlarge` instances
...
OS of BA Postgres is supported by the Canonical team up until the year 2025 (2023 for Ubuntu 18.04).
...
BA Postgres can now be easily upgraded whenever a new major or minor version of PostgreSQL is released. When PostgreSQL 14 comes out,
expect BA Postgres 14 to quickly follow.
...
Last but not the least, we're diversifying the images of BA Postgres available in the AWS and Digital Ocean Marketplaces.
...
| ----------------------------------- | ------------ | ----------- | ----------- |
| BA Postgres                   | ✅           |             |             |
| BA Postgres: PgBouncer Bundle | ✅           | ✅          |             |
| BA Postgres: PostgREST Bundle | ✅           |             | ✅          |
| BA Postgres: Complete Bundle  | ✅           | ✅          | ✅          |

Each offering will be available for both the `x86` and `arm` architectures.

## Try PostgreSQL 13

Try the new features of PostgreSQL 13.3 today by creating a [new project](https://www.assistance.bg/dashboard/) on BA.

## More Postgres resources

- [Realtime Postgres RLS on BA](https://www.assistance.bg//blog/realtime-row-level-security-in-postgresql)
- [Implementing "seen by" functionality with Postgres](https://www.assistance.bg/blog/seen-by-in-postgresql)
- [Partial data dumps using Postgres Row Level Security](https://www.assistance.bg/blog/partial-postgresql-data-dumps-with-rls)
- [Postgres Views](https://www.assistance.bg/blog/postgresql-views)
- [Postgres Auditing in 150 lines of SQL](https://www.assistance.bg/blog/postgres-audit)
- [Cracking PostgreSQL Interview Questions](https://www.assistance.bg/blog/cracking-postgres-interview)
- [What are PostgreSQL Templates?](https://www.assistance.bg/blog/postgresql-templates)
