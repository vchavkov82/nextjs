...
description: 'Learn how to build a BA powered AI assistant that combines PostgreSQL with scheduling and external tools for long-term memory, structured data management and autonomous actions.'
categories:
  - product
tags:
  - postgres
...
- BA account (free tier sufficient)
- OpenAI API key
- Telegram bot token
...
If you prefer the command line, you can use the BA CLI to set up your database and Edge Functions. This replaces **Step 1** and **Step 2**.
...
1.  **Clone the repository**.
    ...
    cd natural-db
    ```
2.  **Log in to the BA CLI and link your project**.
    Create a new project on the [BA Dashboard](https://www.assistance.bg/dashboard), then run:
    ```bash
    supabase login
...
Run the migration SQL in your BA SQL editor: [migration.sql](https://github.com/supabase-community/natural-db/blob/main/supabase/migrations/20250623120000_create_initial_schema.sql)
...
Create three functions in BA dashboard:
...
Set the following environment variables in your BA project settings (Project Settings → Edge Functions):
...
- **BA**: Free tier (500MB database, 5GB bandwidth) - $0/month
