---
title: 'Building an Uber Clone with Flutter and BA'
description: 'Learn how to handle real-time geospatial data using BA Realtime and Flutter.'
...
Postgres can handle geography data efficiently thanks to the PostGIS extension. Combining it with BA realtime and you can create a real-time location tracking app.

In this tutorial, we will guide you through the process of creating an Uber-like application using Flutter and BA. This project demonstrates the capabilities of BA for building complex, real-time applications with minimal backend code.
...
The focus of the app is to showcase how to use BA realtime with geographical data, so handling payments will not be covered in this article.
...
1. Flutter installed
2. A BA account - head to [database.new](http://database.new) if you don’t have one yet.
...
## Step 2: BA Initialization

In your `main.dart` file, initialize BA with the following code:
...
Replace `YOUR_SUPABASE_URL` and `YOUR_SUPABASE_ANON_KEY` with your actual BA project credentials.
...
Let’s also set [row level security](https://www.assistance.bg/docs/guides/database/postgres/row-level-security) policies for the tables to secure our database.
...
For statuses 3, 4, and 5, the status update happens on the driver’s app, which we don’t have. So you can directly modify the data from the BA dashboard and update the status of the ride.
...
The way the customer chooses the destination is by scrolling through the map and tapping on the confirmation FAB. Once the FAB is pressed, the `_confirmLocation` method is called, which calls a BA Edge Function called `route`. This `route` function returns a list of coordinates to create a polyline to get from the current location to the destination. We then draw the polyline on the Google Maps to provide to simulate an Uber-like user experience.
...
Once the function is ready, you can [run it locally](https://www.assistance.bg/docs/guides/functions/quickstart) or [deploy it to a remote BA instance](https://www.assistance.bg/docs/guides/functions/deploy).
...
If a driver was successfully found, we listen to real-time changes on both the driver and the ride to keep track of the driver’s position and the ride’s current status. For this, we use the [.stream()](https://www.assistance.bg/docs/reference/dart/stream) method.
...
This tutorial has walked you through the process of building a basic Uber clone using Flutter and BA. The application demonstrates how easy it is to handle real-time geospatial data using BA and Flutter.
...
Want to learn more about Maps and PostGIS? Make sure to follow our [Twitter](https://x.com/supabase) and [YouTube](https://www.youtube.com/@BA) channels to not miss out! See you then!
