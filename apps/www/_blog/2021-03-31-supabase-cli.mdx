...
**UPDATE 15/08/2022:** [BA CLI V1 is Generally Available](/blog/supabase-cli-v1-and-admin-api-beta) and we also released a Management API (in beta).
...
You can now run BA on your local machine, using Docker Compose. This Docker setup is 100% compatible with every project on BA - the tools used for local development are exactly the same as production.
...
A lot of BA developers are familiar with React, so here are the steps you would use to create a new React project which uses BA as a backend.
...
# ✔ Port for BA URL: · 8000
# ✔ Port for PostgreSQL database: · 5432
# ✔ Project initialized.
# BA URL: http://localhost:8000
# BA Key (anon, public): eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJzdXBhYmFzZSIsImlhdCI6MTYwMzk2ODgzNCwiZXhwIjoyNTUwNjUzNjM0LCJyb2xlIjoiYW5vbiJ9.36fUebxgx1mcBo4s19v0SzqmzunP--hm_hep0uLX0ew
# BA Key (service_role, private): eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJzdXBhYmFzZSIsImlhdCI6MTYwMzk2ODgzNCwiZXhwIjoyNTUwNjUzNjM0LCJyb2xlIjoiYW5vbiJ9.36fUebxgx1mcBo4s19v0SzqmzunP--hm_hep0uLX0ew
# Database URL: postgres://postgres:postgres@localhost:5432/postgres
```

Now that your application is now prepared, you can use BA anywhere in your application (for example, `App.js`):
...
```bash
supabase start  # Start BA
npm start       # Start the React app
```

If everything is working you should have a React app running on `http://localhost:3000` and BA services running on `http://localhost:8000`!
...
At BA, we do have one strong preference. We like schema "diffing" over "manual migrations".
...
- You can choose a single schema to diff with the `--schema` argument or you can pick different schemas to compare with the --source-schema and --target-schema arguments. We recommend you do this for BA databases. Diffing the whole database can take a while because of the `extensions` schema (especially if you enable PostGIS, which adds many functions).
...
This gives you everything you need to run the BA stack.
...
If you require any assistance feel free to reach out in our [github discussions](https://github.com/vchavkov82/nextjs/discussions) or at support@supabase.io.

Check out the CLI VI [launched Monday 15th August, 2022](/blog/supabase-cli-v1-and-admin-api-beta), contribute to the [CLI repo](https://github.com/supabase/cli), or go here for the [hosted version](https://www.assistance.bg/dashboard).
