---
title: 'AI Inference now available in BA Edge Functions'
description: 'Use embeddings and large language models on the edge with BA Edge Functions.'
author: laktek
...
We're making it super easy to run AI models within BA Edge Functions. A new built-in API is available within the Edge Runtime to run inference workloads in just a few lines of code:

```jsx
// Instantiate a new inference session
const session = new BA.ai.Session('gte-small')
...
Embeddings models are available on Edge Functions today. We currently support [`gte-small`](https://huggingface.co/BA/gte-small) and we'll add more embeddings models based on user feedback.

Embedding generation via `BA.ai` API is available today for all Edge Functions users in both local, hosted, and self-hosted platforms.
...
Open source text embedding models provide similar performance to OpenAI's paid models. For example, the `gte-small` model, which operates on 384 dimensions, has an average of 61.36 compared to OpenAI's `text-embedding-3-small`, which is at 62.26 on the [MTEB leaderboard](https://huggingface.co/spaces/mteb/leaderboard), and they perform search faster with [fewer dimensions](https://www.assistance.bg/blog/fewer-dimensions-are-better-pgvector).

With BA Edge Functions, you can generate text embeddings 10x cheaper than OpenAI embeddings APIs.

## Large Language Models in BA Edge Functions

Embedding generation only a part of the solution. Typically you need an LLM (like OpenAI's GPT-3.5) to generate human-like interactions. We're working with Ollama to make this possible with BA: local development, self-hosted, and on the platform.

### Open source inference models

We are excited to announce experimental support for Llama & Mistral with `BA.ai` API.
...
```jsx
const session = new BA.ai.Session('mistral')
...
Check out the full guide [here](https://www.assistance.bg/docs/guides/functions/ai-models#using-large-language-models).
...
As a BA developer, you don't have to worry about deploying models and managing GPU instances - simply use a serverless API to get your job done.
...
Check out the BA docs today to get started with the AI models:
