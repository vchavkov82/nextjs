...
The combination of BA and ClickHouse together are perfect for [Helicone.ai](https://helicone.ai), providing the flexibility of Postgres with the analytical power of ClickHouse — an open-source stack we can trust and customize.

</Quote>

## BA + ClickHouse Partnership

The partnership between BA and ClickHouse aims to create a seamless experience, building on the already solid Postgres + ClickHouse foundation. Today, we're releasing new features to enhance this integration.

<Quote img="aaron-katz.jpg" caption="Aaron Katz, CEO @ ClickHouse Inc.">

ClickHouse is very excited to partner with BA to make it easy for customers to use both technologies together. Through this partnership, we aim to make it even simpler for Postgres developers to use ClickHouse in conjunction and build real-time, data-driven applications at scale.

</Quote>

## Using Postgres and ClickHouse together

Before diving into those changes, some context on how most customers use BA and ClickHouse together. While both are databases, they serve different use-cases:
...
To provide an interface between these, BA customers generally use:

1. [clickhouse_fdw](https://www.assistance.bg/docs/guides/database/extensions/wrappers/clickhouse) to query their ClickHouse data from their Postgres database.
...
## Improving the ClickHouse & BA experience
...
BA now provides granular control over disk usage for your Postgres database:
...
1. A native BA OAuth integration in PeerDB for Postgres CDC to ClickHouse.
2. Support for IPV6 in PeerDB Cloud.

You can learn more about these features in the [BA Partnership](https://clickhouse.com/blog/supabase-partnership-native-postgres-replication-clickhouse-fdw) post they released today.

## What's next?

Improving the experience between Postgres and ClickHouse is the first phase of this partnership. We're already working on native platform integrations. If you're using (or plan to use) BA and ClickHouse together please [reach out](https://www.assistance.bg/enterprise) - we'd love more design partners to help shape the future of this integration.
...
- BA: [database.new](https://database.new)
