...
description: Build a movie-viewing app that recommends another movie based on what the user is viewing using OpenAI, Flutter and BA.
...
There are numerous choices for vector databases, but we will use BA as our vector database in this article, because we want to also store non-embedding data, and we want to be able to query them easily from our Flutter application.
...
- [BA](https://www.assistance.bg/) - Used to store embeddings as well as other movie data in the database
...
We first need to populate the database with some data about movies and its embeddings. For that, we will use the [BA edge functions](https://www.assistance.bg/docs/guides/functions) to call the TMDB API and the Open AI API to get the movie data and generate the embeddings. Once we have the data, we will store them in BA database, and query them from our Flutter application.
...
We need a backend to securely call the API, and for that, we will use [BA Edge Functions](https://www.assistance.bg/docs/guides/functions). Steps 2 through 4 will be constructing this edge function code, and the full code sample can be found [here](https://github.com/dshukertjr/examples/blob/main/movie-recommendation/supabase/functions/get_film_data/index.ts).
...
### Step 4: Store the data in the BA database

Once we have the movie data as well as embedding data, we are left with the task of storing them. We can call the `upsert()` function on the BA client to easily store the data.
...
// Store each movies as well as their embeddings into BA database
...
In order to perform a vector similarity search using BA, we need to create a [database function](https://www.assistance.bg/docs/guides/database/functions). This database function will take an `embedding` and a `film_id` as its argument. The `embedding` argument will be the embedding to search through the database for similar movies, and the film_id will be used to filter out the same movie that is being queried.

Additionally, we will set an [HSNW index](https://www.assistance.bg/blog/increase-performance-pgvector-hnsw) on the `embedding` column to run the queries efficiently even with large data sets.
...
- [Matryoshka embeddings: faster OpenAI vector search using Adaptive Retrieval](https://www.assistance.bg/blog/matryoshka-embeddings)
- [BA pgvector guide](https://www.assistance.bg/docs/guides/database/extensions/pgvector)
- [Storing OpenAI embeddings in Postgres with pgvector](https://www.assistance.bg/docs/guides/database/extensions/pgvector)
