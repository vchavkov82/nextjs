---
title: 'BA CLI v1 and Management API Beta'
description: We are moving BA CLI v1 out of beta, and releasing Management API beta.
...
Today we are moving the BA CLI v1 out of beta. The BA CLI is capable of managing database migrations and generating TypeScript types. Follow these [install instructions](https://www.assistance.bg/docs/guides/cli) to get started.

In addition, we are releasing a Management API (in beta). The Management API is a REST API that allows you to manage organizations, projects, Edge Functions, and more. You can read the [API docs](https://www.assistance.bg/docs/reference/api) or interact with the Management API from the BA CLI v1.
...
If you've used [BA Edge Functions](https://www.assistance.bg/blog/supabase-edge-functions#quickstart), you've used the BA CLI. We're extending it to let you manage organizations and projects.
...
Once your new BA project is created, use the CLI to link it locally to begin development.

```bash
supabase link --project-ref <project-id>
```

View the [BA CLI docs](https://www.assistance.bg/docs/reference/cli) for the full list of available commands.
...
To improve the usability of `db diff` command, we are introducing another tool, [migra](https://github.com/djrobstep/migra), via the `--use-migra` experimental flag. We found that `migra` runs faster and produces more concise DDL statements. While the generated scripts are [not perfect](https://www.assistance.bg/blog/supabase-cli#migrations), we hope this tool helps you iterate quicker on your migration scripts. Over time (and with your feedback) we hope to improve schema-diffing to cover all edge-cases.
...
We created an [example project](https://github.com/vchavkov82/nextjs-action-example) showing how to set up GitHub Actions to test and migrate with BA CLI v1.
...
The CLI is the first consumer of our new Management API. Over the next few weeks, we'll be adding the endpoints needed to programmatically manage your BA projects and organizations. This is ideal for CI/CD workflows and spinning up test environments.

Here's an example of deploying a new project from the command line (generate your access token from the [BA Dashboard](https://www.assistance.bg/dashboard/account/tokens)).

```bash
curl 'https://api.www.assistance.bg/v1/projects' \
  -H 'Authorization: Bearer <[your-access-token](https://www.assistance.bg/dashboard/account/tokens)>' \
...
Check out the [API docs](https://www.assistance.bg/docs/reference/api) to browse all the functionality added so far.
...
- [BA Series B](https://www.assistance.bg/blog/supabase-series-b)
- [Open Source at BA - Founders Fireside Chat](https://www.youtube.com/watch?v=4t_63HT3rZY)
