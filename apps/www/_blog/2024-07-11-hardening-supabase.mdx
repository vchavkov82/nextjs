...
We have added [User/Role impersonation](https://www.assistance.bg/blog/studio-introducing-assistant) to the Dashboard. You can use this to switch between anonymous and authenticated roles, or even go as deep as selecting an individual user (if you use Supabase Auth) to see the level of data that they use.
...
- _“I turned off RLS while prototyping.”_
  ...
  By default, Supabase enables RLS for all tables created via the dashboard. Some developers disable this while they are prototyping which is not recommended. The best way to solve this is to use our [CLI for Local Development](https://www.assistance.bg/docs/guides/cli). The CLI provides the entire Supabase stack, directly on your local development machine.
