...
PostgREST turns your PostgreSQL database automatically into a RESTful API. Today, PostgREST v10 was [released](https://github.com/PostgREST/postgrest/releases/tag/v10.0.0). v10 is not available on the BA Platform yet, but it is available for [self-hosting](https://www.assistance.bg/docs/guides/hosting/overview) or as a executable binary from the GitHub release page.

Let's take a look at some of its new features that go hand in hand with [supabase-js v2](https://www.assistance.bg/blog/supabase-js-v2).
...
`explain()` is also useful for getting the [pg_stat_statements](https://www.postgresql.org/docs/current/pgstatstatements.html) query identifier, which you can use in the BA logs to obtain the generated SQL queries. Here we use the json `format` and the `verbose` option to get it.
...
We’ll release PostgREST 10 on the BA platform over the next month.
