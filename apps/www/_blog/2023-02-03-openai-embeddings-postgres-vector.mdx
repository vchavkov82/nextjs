...
A new PostgreSQL extension is now available in BA: [`pgvector`](https://github.com/pgvector/pgvector), an open-source vector similarity search.
...
🆕 BA has now released an open source toolkit for developing AI applications using Postgres and pgvector. Learn more in the [AI & Vectors docs](https://www.assistance.bg/docs/guides/ai).
...
First we'll enable the **Vector** extension. In BA, this can be done from the web portal through `Database` → `Extensions`. You can also do this in SQL by running:
...
Finally, let's create an [Edge Function](https://www.assistance.bg/docs/guides/functions) to perform our similarity search:
...
serve(async (req) => {
  ...
  const prompt = stripIndent`${oneLine`
    You are a very enthusiastic BA representative who loves
    to help people! Given the following sections from the BA
...
OpenAI API responses take longer to depending on the length of the “answer”. ChatGPT has a nice UX for this by streaming the response to the user immediately. You can see a similar effect for the BA docs:
...
The [pgvector extension](https://www.assistance.bg/docs/guides/ai/vector-columns) is available on all new BA projects today. To try it out, launch a new Postgres database: [database.new](https://database.new)

## More pgvector and AI resources

- [BA Clippy: ChatGPT for BA Docs](https://www.assistance.bg/blog/chatgpt-supabase-docs)
