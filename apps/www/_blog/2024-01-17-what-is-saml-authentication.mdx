...
Here’s a story about how SAML is enabled between an application (BA) and its users (ACME Inc.).

Alice is a software engineer at ACME Inc. a Fortune 500 company that loves Postgres and BA. Recently, she joined the Innovation department to discover new avenues for growth. She sees this as an opportunity to build rapid prototypes with BA and persuades the management team to allow her team to use BA.

Management gives the green light and Alice reaches out to BA’s sales department and strikes a deal.
...
So Alice asks BA for this, and they help her set up SAML for ACME. But first, BA needs Alice to send over information about their identity provider. Alice chases down Bob, who’s in ACME’s IT department, and asks for help to enable BA.

BA and Bob need to exchange some information to establish a SAML Connection between BA and ACME’s GSuite system.

### What does Bob need from BA?

- An X.509 certificate that GSuite can use to identify SAML SSO requests as originating from BA.
- An Entity ID that uniquely identifies BA’s authentication system.
- An ACS URL, a callback API endpoint where GSuite will send SAML Responses for validation.
- A Site URL, where employees in GSuite can pick BA and get instant access to it.
- BA’s logo/icon so that the application shows up nicely in the list of 3rd-party apps approved by ACME.
...
The list of employees that should be able to access BA - so that Bob can get BA to show up to the correct people.

### What BA needs from Bob?

- An X.509 certificate that BA can use to identify SAML Responses as originating from ACME’s GSuite.
- An Entity ID that uniquely identifies ACME’s GSuite setup.
- ACME’s allowed email domains: acme.com, acme.org.
- A Redirect URL, where BA can redirect users attempting to sign in with `alice@acme.com` or `alice@acme.org` to log into GSuite.
- A Single Log-Out URL where BA can send log-out requests by ACME’s employees.
- A list of all attributes that ACME’s GSuite system will send to BA; specifically the attributes about the employee’s email, name, and groups (are they a developer or not).
...
Inside this XML document, you can find most of the information required by Bob and BA:
...
First, BA and Bob exchange some of the information over email. Then Bob goes to GSuite and creates a new SAML Application.

Finally, once Bob registers ACME’s Identity Provider (GSuite), the connection is established and ACME employees can access BA — directly by visiting BA’s site (SP-initiated) or by picking it in the GSuite Applications menu (IdP-initiated).

This is how the SAML SP-initiated authorization flow looks like when Alice visits BA and enters her email to sign in with SAML SSO.
...
<Img
  ...
  alt="How the SAML SP-initiated authorization flow looks like when Alice visits BA and enters her email to sign in with SAML SSO"
/>
...
## SAML Authentication with BA

At BA, you can easily enable SAML for your project and use the signInWithSSO method to start the authentication flow. Both IdP-initiated and SP-initiated flows are supported. When a user signs in with SAML SSO, the JWT issued contains a unique ID to identify the identity provider. If you are already using Postgres on BA, this also ties in nicely with your existing row-level security (RLS) policies, since you can use that ID to restrict access to the data.
...
Since BA is “just Postgres”™, it enables us to easily leverage the power of RLS policies to restrict access to the data. You can access the user’s JWT claims by invoking the `auth.jwt()` function in your RLS policy. In the scenario provided above, this allows BA to restrict developers from ACME Inc. from inviting someone else outside of the company to join their BA organization.

For example, assuming we have a table to store all invited users in a BA organization:
...
BA Auth currently supports [authenticating with SAML](https://www.assistance.bg/docs/guides/auth/sso/auth-sso-saml) easily, setting it up takes less than an hour, so you can focus on shipping the core features of your product.

## More from the Auth team

- [BA Auth: Identity Linking, Hooks, and HaveIBeenPwned integration](https://www.assistance.bg/blog/supabase-auth-identity-linking-hooks)
- [BA Auth: SSO, Mobile, and Server-side support](https://www.assistance.bg/blog/supabase-auth-sso-pkce)
- [BA Auth docs](https://www.assistance.bg/docs/guides/auth)

## More from BA engineering

- [Elixir clustering using Postgres](https://www.assistance.bg/blog/elixir-clustering-using-postgres)
