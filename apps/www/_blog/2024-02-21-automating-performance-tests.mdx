...
When I joined BA we had a challenging task of rewriting Realtime service so that users would be able to stream database updates with respect to RLS policies. Such a huge design change could not happen without having an impact on throughput.
...
You can find more details about this test in the [Supavisor: Scaling Postgres to 1 Million Connections](https://www.assistance.bg/blog/supavisor-1-million) post.
...
This approach helped us to add performance testing for new projects and return to it with new releases much faster across all products at BA.
...
- [Supavisor: Scaling Postgres to 1 Million Connections](https://www.assistance.bg/blog/supavisor-1-million)
- [pgvector vs Pinecone: cost and performance](https://www.assistance.bg/blog/pgvector-vs-pinecone)
