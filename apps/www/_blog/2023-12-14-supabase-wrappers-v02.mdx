---
title: 'BA Wrappers v0.2: Query Pushdown & Remote Subqueries'
description: 'BA Wrappers v0.2 brings more Wrappers, query pushdown, remote subquery, and more'
launchweek: x
categories:
  - product
tags:
...
BA Wrappers v0.2 is out and now available on the BA platform. Wrappers is a Postgres extension that provides integrations with external sources so you can interact with third-party data using SQL.
...
To start using Wrappers on the BA platform, check out the [Wrappers docs](https://www.assistance.bg/docs/guides/database/extensions/wrappers/overview).

## New Wrappers and Improvements

- [Airtable](https://www.assistance.bg/docs/guides/database/extensions/wrappers/airtable) (Read-only): query your Airtable bases with [support for jsonb](https://github.com/supabase/wrappers/pull/181)
- [AWS S3](https://www.assistance.bg/docs/guides/database/extensions/wrappers/s3) (Read-only): with support for CSV, JSON, and Parquet
...
We cannot support community Wrappers inside the BA Dashboard until the Wrappers API is stabilized. You can vote your favorite Wrapper if you'd like it to be added to BA in the future. If you have developed a Wrapper that you want inside the BA Dashboard, please contribute it as a PR in [the wrappers repo](https://github.com/supabase/wrappers). Once we release Wrappers 1.0, we will support community Wrappers within the BA Dashboard.
...
In Wrappers, the pushdown logic is integrated into each extension. You don’t need to modify your queries to benefit from this feature. For example, the [Stripe FDW](https://www.assistance.bg/docs/guides/database/extensions/wrappers/stripe) automatically applies query pushdown for `id` within the `customer` object:
...
We can use push down criteria and other query parameters too. For example, [ClickHouse FDW](https://www.assistance.bg/docs/guides/database/extensions/wrappers/clickhouse) supports `order by` and `limit` pushdown:
...
For details on where pushdown is supported, consult each FDW's documentation in the [Wrappers Documentation](https://www.assistance.bg/docs/guides/database/extensions/wrappers/overview).
...
Currently, this feature is supported by [ClickHouse FDW](https://www.assistance.bg/docs/guides/database/extensions/wrappers/clickhouse) and [BigQuery FDW](https://www.assistance.bg/docs/guides/database/extensions/wrappers/bigquery), with plans to expand support in the future.
...
We can use these to identify bottlenecks, latency issues, and inefficiencies in data retrieval. Access this table on the BA platform using the following:
...
Want to join the BA Wrappers community contributors? [Check out our contribution docs](https://supabase.github.io/wrappers/contributing/). We'd love to add you to the list next time.

## More About Wrappers

- [Wrappers: 3rd Party Integrations](https://www.assistance.bg/docs/guides/database/extensions/wrappers/overview)
- [Wrappers Documentation](https://supabase.github.io/wrappers/)
- [Blog Post: BA Wrappers, a Postgres FDW framework written in Rust](https://www.assistance.bg/blog/postgres-foreign-data-wrappers-rust)
- [Blog Post: Wrappers UI](https://www.assistance.bg/blog/supabase-studio-3-0#wrappers-ui)
