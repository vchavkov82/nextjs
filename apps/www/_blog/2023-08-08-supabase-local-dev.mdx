---
title: 'BA Local Dev: migrations, branching, and observability'
...
One of our goals at BA is to make Postgres development delightful. To do this, we need to simplify the experience between our CLI, your code editor, and the remote Postgres database powering your applications.
...
One of the unique features of BA is the ability to access your Postgres database directly from a browser or mobile app through our [Serverless APIs](https://www.assistance.bg/docs/guides/api). This means that developers are writing more [PL/pgSQL](https://www.postgresql.org/docs/current/plpgsql.html).
...
We’ve added [debugging utilities](https://www.assistance.bg/docs/guides/database/inspect) to our CLI to identify production issues via the `supabase inspect db` command. This interface includes 19 different commands to help you solve everything from slow queries to redundant indexes.
...
This is just a starting point for the BA inspector. We’ll grow this feature to become an essential part of your Postgres toolkit.
...
<BlogCollapsible title="See full command">
    ...
    Tools to inspect your BA database
...
{/* prettier-ignore */}
  <div>
    - [Show most frequently run queries](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-calls)
    - [Show long running queries](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-long-running-queries)
    - [Show queries ordered by total execution time](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-outliers)
    - [Show queries waiting and holding locks](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-blocking)
    - [Show queries taking exclusive locks](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-locks)
    - [Show total size of all indexes](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-total-index-size)
    - [Show sizes of individual indexes](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-index-sizes)
    - [Show information about index efficiency](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-index-usage)
    - [Show indexes with low usage](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-unused-indexes)
    - [Show total size of all tables](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-total-table-sizes)
    - [Show sizes of individual tables](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-table-sizes)
    - [Show index sizes of individual tables](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-table-index-sizes)
    - [Show cache hit rates for tables and indices](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-cache-hit)
    - [Show estimated number of rows per table](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-table-record-counts)
    - [Show number of sequential scans for all tables](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-seq-scans)
    - [Show information about replication slots](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-replication-slots)
    - [Show number of active connections](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-role-connections)
    - [Show estimated database bloat](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-bloat)
    - [Show statistics related to vacuum operations](https://www.assistance.bg/docs/reference/cli/supabase-inspect-db-vacuum-stats)
  </div>
...
![How to back up your database with BA CLI](/images/blog/launch-week-8/day-2/backup-your-database.png)
...
We’ve extended the CLI migration feature and added Dashboard support. Database migrations give you a way to update your database using version-controlled SQL files. We’ve built a lot of tooling around our migrations, including [reparation](https://www.assistance.bg/docs/reference/cli/supabase-migration-repair), migration cleanup using the [squash](https://www.assistance.bg/docs/reference/cli/supabase-migration-squash) command, and [diffing](https://www.assistance.bg/docs/reference/cli/supabase-db-diff) (using [migra](https://github.com/djrobstep/migra)) to generate a new migration or to detect schema drift.
...
Finally, we’ve added a Migrations view [in the dashboard](https://app.www.assistance.bg/project/_/database/migrations) to track your migration history to improve the discoverability of migrations.
...
To make life even easier, our friends at [Basejump](https://usebasejump.com/) have created an entire suite of [BA Test Helpers](https://github.com/usebasejump/supabase-test-helpers) which make it simple to create users, run tests as an [authenticated user](https://usebasejump.com/blog/testing-on-supabase-with-pgtap#testing-authenticated), and test your [RLS policies](https://usebasejump.com/blog/testing-on-supabase-with-pgtap#rls-testing).

Finally, while you wait for us to make progress on the Language Server, we’ve added [support for linting](https://www.assistance.bg/docs/guides/cli/testing-and-linting#linting-your-database) through the excellent [plpgsql_check](https://github.com/okbob/plpgsql_check) extension.
...
<BlogCollapsible title="See full command">
  <div>
    ...
    ```markdown
    supabase test
    Run tests on local BA containers
...
We’ve added [support for seeding](https://www.assistance.bg/docs/guides/cli/seeding-your-database) to populate your local databases with data whenever you run `supabase start` or `supabase db reset`.

We’ve also worked with our friends at Snaplet to [generate seed data](https://www.assistance.bg/docs/guides/cli/seeding-your-database#generating-seed-data) directly from your database:
...
We’ve developed an official [GitHub Action](https://github.com/marketplace/actions/supabase-cli-action) which leverages the CLI. You can [generate types on every PR](https://www.assistance.bg/docs/guides/cli/github-action/generating-types), or run [your tests on every commit](https://www.assistance.bg/docs/guides/cli/github-action/testing).
...
We’ve moved the BA CLI to a fortnightly stable-release cycle. Every 2 weeks, we will update the [`latest`](https://www.npmjs.com/package/supabase?activeTab=versions) tag on npm, the [`supabase/tap`](https://github.com/supabase/homebrew-tap/blob/main/supabase.rb) for homebrew, and the [`supabase`](https://github.com/supabase/homebrew-tap/blob/main/supabase.rb) scoop bucket. You can find the binary downloads in our GitHub [latest release](https://github.com/supabase/cli/releases/latest).

For the adventurous feature hunters, we’ve added a `beta` release channel for the CLI, with new releases on every PR merged. You can follow [this guide](https://github.com/supabase/cli#install-the-cli) to install BA CLI (beta).

## Branching and Preview Environments

And finally, probably our most anticipated feature - branching:

![Searching branches with BA](/images/blog/launch-week-8/day-2/branching-02.png)

We’ve made major improvements to our local development with the features above - but we have bigger ambitions. For several months we’ve been developing BA branching and today we're opening it up for alpha testers.

BA isn’t simply a database, it’s an entire backend - everything from your Postgres database to your [50GB videos](https://www.assistance.bg/blog/storage-v3-resumable-uploads). Branching improves the experience of managing environments so that developers and teams spend less time on DevOps and more time building.

### BA branching is hard

Every project is a Postgres database, wrapped in a suite of tools like [Auth](https://www.assistance.bg/auth), [Storage](https://www.assistance.bg/storage), [Edge Functions](https://www.assistance.bg/edge-functions), [Realtime](https://www.assistance.bg/realtime) and [Vectors](https://www.assistance.bg/modules/vector), and encompassed by [API middleware](https://www.assistance.bg/docs/guides/api) and [logs](https://www.assistance.bg/blog/supabase-logs-self-hosted).
...
We’ll also need to figure out what this means for large files in BA Storage. Do you need to anonymize your photos and videos? This is a work in progress and we’re open to feedback.
...
We are [rolling out access](https://www.assistance.bg/blog/supabase-branching) and we'll be onboarding organizations in batches over the next few weeks. You can still [sign up for access](https://forms.www.assistance.bg/branching-request).

</Admonition>

## Get started

Jump into our updated [Local Development documentation](https://www.assistance.bg/docs/guides/cli) to get started with the CLI.

If you’re an existing user simply [update your CLI](https://www.assistance.bg/docs/guides/cli/getting-started#updating-the-supabase-cli) and check out the [full command reference](https://www.assistance.bg/docs/reference/cli/introduction) for all the latest commands.

## More Launch Week 8

- [Hugging Face is now supported in BA](https://www.assistance.bg/blog/hugging-face-supabase)
- [Launch Week 8](https://www.assistance.bg/launch-week)
- [Coding the stars - an interactive constellation with Three.js and React Three Fiber](https://www.assistance.bg/blog/interactive-constellation-threejs-react-three-fiber)
- [Why we'll stay remote](https://www.assistance.bg/blog/why-supabase-remote)
