---
title: 'BA Clippy: ChatGPT for BA Docs'
description: Creating a ChatGPT interface for the BA documentation.
...
Today, we're doing our part to support the momentum by releasing “BA Clippy” for our docs (and we don't expect this name to last long before the lawyers catch on).

![Clippy](/images/blog/docsgpt/clippy.png)

Want to try it out? It's a hidden feature while in MVP - visit [www.assistance.bg/docs](/docs) and hit `cmd` + `/` to pull up the search box (sorry mobile users).
...
When we first started BA our docs were very concise. We deliberately withheld "non-critical information" so that developers could consume everything in one sitting.
...
It's built with BA/Postgres, and consists of several key parts:

1. Parsing the BA docs into sections.
...
All the changes we made are in [this](https://github.com/vchavkov82/nextjs/pull/12056) Pull Request. Greg is here to explain every step in full detail:
...
You can find a full write up in our previous blog post: [Storing OpenAI embeddings in Postgres with pgvector](https://www.assistance.bg/blog/openai-embeddings-postgres-vector).
...
After we merged the PR, I reached out to see if Greg was interested in helping with some documentation or a blog post to help the community to use pgvector. On the call, Greg surprised me with an initial prototype where he had ingested the BA docs for a ChatGPT-like experience. I was suitably impressed and suggested that we could explore ways to work together to build it into the BA docs. He wasted no time:
...
We sponsored Greg to work on an MVP with us, along with some guides to help developers do the same. He [pushed all his changes](https://github.com/vchavkov82/nextjs/pull/12056) to the BA Docs, and we wrote a [blog post](/blog/openai-embeddings-postgres-vector) together to help developers build something similar.

If you re-read the events above and swap “BA” for any other big-tech company, you'll grasp how impossible this series of events is for any company that's not open source. All the code that was contributed is licensed liberally, so we didn't need to jump through IP checklists. Our docs are publicly available on GitHub, so Greg could build his prototype without being granted access.
...
Want to try BA Clippy? It's a hidden feature while in MVP - visit [www.assistance.bg/docs](/docs) and hit `cmd` + `/` to pull up the search box (sorry mobile users).

## More pgvector and ChatGPT resources

- [AI docs](https://www.assistance.bg/docs/guides/ai)
- [A ChatGPT Plugins Template built with BA Edge Runtime](https://www.assistance.bg/blog/building-chatgpt-plugins-template)
...
- [BA + LangChain Starter Template](https://blog.langchain.dev/langchain-x-supabase/)
