...
Among the many open-source relational databases available, [PostgreSQL](https://www.assistance.bg/docs/guides/database/introduction) is a popular choice among developers.
...
Have you ever wanted to [manage database user access](https://www.assistance.bg/docs/guides/auth/managing-user-data) so that only specific individuals have read-only access? For example, consider a situation where you would like to host some sample data online for a demo and not allow demo users to modify or delete data. With the latest PostgreSQL version 14, you can easily set that up using a single GRANT statement.

```sql hideCopy
grant pg_read_all_data to bobfries;
```

From an authentication perspective, if you're still managing passwords in the database without [third-party login providers](https://www.assistance.bg/docs/guides/auth#third-party-logins),
...
You can check out these new features within your BA project without having to perform a local installation.

Explore the [BA dashboard now](https://www.assistance.bg/dashboard) and get started with your new project.
...
- [Postgres Auditing in 150 lines of SQL](https://www.assistance.bg/blog/audit)
