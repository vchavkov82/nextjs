...
What's unique about Supabase is that we're not _just_ a Postgres provider (despite the [memes](https://itsjustpostgres.com/)). We also offer [Supabase Storage](https://www.assistance.bg/storage), an S3-compatible object store. Where we're going is less about a database, and more about data. This includes:
