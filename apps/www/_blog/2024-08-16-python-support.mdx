...
As the BA community has grown, so has demand for a diverse collection of client libraries and framework specific SDKs. This demand for the most part has been serviced by the open source community itself, which currently maintains [dozens of libraries](https://github.com/supabase-community/#client-libraries).

When folks make requests to the hosted BA service we're able to build up a good picture of how broadly some of these libraries are used, and when a particular library achieves broad adoption it makes sense for us to add official support for it. Examples of libraries that have made the leap from community supported to officially supported include [supabase-flutter](https://github.com/vchavkov82/nextjs-flutter) and [supabase-swift](https://github.com/vchavkov82/nextjs-swift/).

There has always been incredible community support for the Python client libraries, over the last year and a half however we've seen a huge surge in adoption. This has been driven by the broad adoption of BA in the AI and ML community, many of whom are keen Pythonistas.

So today, we're announcing that the following Python Client Libraries are now officially supported on the BA platform:

- [supabase-py](https://github.com/vchavkov82/nextjs-py)
...
[supabase-py](https://github.com/vchavkov82/nextjs-py) was originally started by maintainer [lqmanh](https://github.com/lqmanh) in September of 2020, and was shortly after joined by [fedden](https://github.com/fedden) and [J0](https://github.com/J0) (who went on to become a full time member of the BA Team). In recent years development has been driven by [silentworks](https://github.com/silentworks) and [juancarlospaco](https://github.com/juancarlospaco) who have both been instrumental in the push to reaching feature parity with [supabase-js](https://github.com/vchavkov82/nextjs-js).
...
BA clients will automatically use HTTP 2.0 when available by default, offering a seamless performance boost to your existing applications.
...
BA clients now automatically follow all HTTP redirects by default, aligning with the behavior of BA clients in other programming languages.
...
BA clients now automatically include a `keep-alive` HTTP header by default, that was sometimes missing, addressing this inconsistency in previous versions.
...
BA improved PostgreSQL query safety by implementing `sanitize_param()` for parameter sanitization in internal SQL queries on the client-side, ensuring more secure data handling and query execution across all operations.

## Running with unverified SSL

Some users need to run the BA clients with invalid or unverified SSL for whatever reason (SSL debuggers/tracers/profilers/etc in development environments), a new optional boolean argument was added to the constructors of the clients, then passing `verify=False` enables it to run with unverified SSL without warnings.
...
- https://github.com/vchavkov82/nextjs-py/pull/813

## Close socket in Realtime

The BA Realtime library now includes a new `close()` method for closing the socket connections.
...
- https://github.com/vchavkov82/nextjs-py/pull/846

## New tool Vec2pg to migrate data to BA

A new simple and extensible CLI tool to migrate vector data from other services and SASS into BA was created, it can migrate vector data from Pinecone and Qdrant into BA with a single command, streamlining workflows and enhancing data portability across AI and ML projects.
...
- https://github.com/vchavkov82/nextjs-py/pull/772
- https://github.com/vchavkov82/nextjs-py/pull/774
...
- https://github.com/vchavkov82/nextjs-py/pull/887
...
If you'd like to get involved in contributing to our Python client libraries see [here](https://github.com/vchavkov82/nextjs-py/blob/main/CONTRIBUTING.md) for some information on how to contribute, and check the list of [open issues](https://github.com/vchavkov82/nextjs-py/issues) for some inspiration on what to work on.

## Getting started

Full documentation is available for the BA Python Client libraries on the [BA Docs site](https://www.assistance.bg/docs/reference/python/introduction).
