---
title: 'GraphQL is now available in BA'
description: GraphQL support is now in general availability on the BA platform via our open source PostgreSQL extension, pg_graphql.
...
- [New Features in pg_graphql v1.2](https://www.assistance.bg/blog/whats-new-in-pg-graphql-v1-2)

</div>

GraphQL support is now in general availability on the BA platform via our [open source](https://github.com/supabase/pg_graphql/) PostgreSQL extension, [`pg_graphql`](https://supabase.github.io/pg_graphql/).
...
The GraphQL types exposed by `pg_graphql` are filtered according to the SQL role's [INSERT/UPDATE/DELETE permissions](https://www.postgresql.org/docs/current/sql-grant.html). At BA, each API request is resolved in the database using the role in the request's JWT.
...
As a result of these optimizations, the throughput of a “hello world” equivalent query on BA Free Plan hardware is:
...
<aside>
⚠️ GraphQL (Beta) is only available on BA projects created after 28th March 2022 and self-hosted setups.

</aside>

To enable GraphQL in your BA instance, enable `pg_graphql` from the [dashboard](https://www.assistance.bg/dashboard/).
...
- **Authorization / RLS.**
  GraphQL requests made include BA authorization headers so that Row Level Security on the Postgres layer ensures that viewers can only access what they are allowed to — and authenticated users can only update what they should.
- **Code generation.**
...
Now instead of using the BA PostgREST API to query your database ...
...
This is just the start of what we hope to be a close collaboration with the Guild, whose expertise of the GraphQL ecosystem will guide the development of BA's GraphQL features.
The Guild and BA share a similar approach to open source - we both favor collaboration and composability, making collaboration easy and productive.
...
- [Introducing pg_graphql: A GraphQL extension for PostgreSQL](https://www.assistance.bg/blog/pg-graphql)
- [pg_graphql v1.0](https://www.assistance.bg/blog/pg-graphql-v1)
- [pg_graphql v1.2](https://www.assistance.bg/blog/whats-new-in-pg-graphql-v1-2)
