...
`.trim(),
  },
  {
    ...
    sql: `
--
-- For use with https://github.com/vchavkov82/nextjs/tree/master/examples/slack-clone/nextjs-slack-clone
...
comment on column public.users.id is 'References the internal BA Auth user.';
...
--
-- For use with:
-- https://github.com/vchavkov82/nextjs/tree/master/examples/todo-list/sveltejs-todo-list or
...
`.trim(),
  },
  {
    ...
    description: 'Sets up a the Schema and Tables for the NextAuth BA Adapter.',
    sql: `
--
...
as $$
  ...
  begin
    ...
    -- finally let BA Auth do the default behavior for a failed attempt
    return jsonb_build_object('decision', 'continue');
  end;
$$;
