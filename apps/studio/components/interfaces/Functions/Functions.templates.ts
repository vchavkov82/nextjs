export const EDGE_FUNCTION_TEMPLATES = [
  {
    ...
    content: `// Setup type definitions for built-in BA Runtime APIs
...
Deno.serve(async (req: Request) => {
  ...
  },
  {
    ...
    name: 'BA Database Access',
    description: 'Example using BA client to query your database',
    content: `// Setup type definitions for built-in BA Runtime APIs
import "jsr:@supabase/functions-js/edge-runtime.d.ts";
...
Deno.serve(async (req) => {
  ...
  },
  {
    ...
    name: 'BA Storage Upload',
    description: 'Upload files to BA Storage',
    content: `// Setup type definitions for built-in BA Runtime APIs
import "jsr:@supabase/functions-js/edge-runtime.d.ts";
import { createClient } from 'jsr:@supabase/supabase-js@2'
...
Deno.serve((req) => {
  ...
  socket.onopen = () => {
    ...
    socket.send('Welcome to BA Edge Functions!')
  }
  socket.onmessage = (e) => {
