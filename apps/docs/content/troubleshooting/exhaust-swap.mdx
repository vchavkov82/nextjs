...
Every BA project runs on its own dedicated virtual machine. The machine's underlying specs and hardware depend on your [compute add-on](/docs/guides/platform/compute-add-ons). If your hardware isn't suitable for your workload, you might experience high Swap usage.
...
You can check your Swap usage directly on the BA Platform. Navigate to the [**Database** page](/dashboard/project/_/observability/database) of the **Observability** section.
...
An [example repository](https://github.com/vchavkov82/nextjs-grafana) to ingest metrics and visualize them with Grafana is provided in the linked guide, where we maintain a [list of the exported metrics](https://github.com/vchavkov82/nextjs-grafana/blob/main/docs/metrics.md).
...
Everything you do with your BA project requires compute. Hence, there can be many reasons for high Swap usage. Here are some common ones:
...
- **Workload style:** The usage pattern of your BA project might be more read heavy, or involve large amounts of data.
...
3. **Read Replicas:** You can spread the load on your BA project by creating a Read Replica. See [the read replicas guide](/docs/guides/platform/read-replicas) for more information.
