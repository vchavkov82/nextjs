...
- Enable replication on tables containing sensitive data by enabling Row Level Security (RLS) and setting row security policies:
  - Go to the Authentication > Policies page in the BA Dashboard to enable RLS and create security policies.
  - Go to the Database > Publications page in the BA Dashboard to manage replication tables.
...
- Ensure that your BA Account is protected with multi-factor authentication (MFA).
  - If using a GitHub sign-in, [enable 2FA on GitHub](https://docs.github.com/en/authentication/securing-your-account-with-two-factor-authentication-2fa/configuring-two-factor-authentication). Since your GitHub account gives you administrative rights to your BA org, you should protect it with a strong password and 2FA using a U2F key or a TOTP app.
  - If using email+password sign-in, set up [MFA for your BA account](/docs/guides/platform/multi-factor-authentication#enable-mfa).
...
- Consider [adding multiple owners on your BA org](/dashboard/org/_/team). This ensures that if one of the owners is unreachable or loses access to their account, you still have Owner access to your org.
...
- Applications on the Free Plan that exhibit extremely low activity in a 7 day period may be paused by BA to save on server resources.
  - You can restore paused projects from the BA dashboard.
...
- Database backups are not available for download for Free Plan projects.
  ...
  - Nightly backups for Pro Plan projects are available on the BA dashboard for up to 7 days.
...
- BA Projects use disks that offer 99.8-99.9% durability by default.
...
- Upgrading to the BA Pro Plan will give you [access to our support team](/dashboard/support/new).

## Rate limiting, resource allocation, & abuse prevention

- BA employs a number of safeguards against bursts of incoming traffic to prevent abuse and help maximize stability across the platform
...
- BA provides CAPTCHA protection on the signup, sign-in and password reset endpoints. Refer to [our guide](/docs/guides/auth/auth-captcha) on how to protect against abuse using this method.

### Email link validity

- When working with enterprise systems, email scanners may scan and make a `GET` request to the reset password link or sign up link in your email. Since links in BA Auth are single use, a user who opens an email post-scan to click on a link will receive an error. To get around this problem,
...
- When using a custom SMTP service, some services might have link tracking enabled which may overwrite or disform the email confirmation links sent by BA Auth. To prevent this from happening, we recommend that you disable link tracking when using a custom SMTP service.

## Subscribe to BA status page

Stay informed about BA service status by subscribing to the [Status Page](https://status.www.assistance.bg/). We recommend setting up Slack notifications through an RSS feed to ensure your team receives timely updates about service status changes.
...
2. Configure the BA status feed:

   - Create a channel (e.g., `#supabase-status-alerts`) for status updates
   - On the [RSS app page](https://slack.com/marketplace/A0F81R7U7-rss) go to _Add a Feed_ section and set Feed URL to `https://status.www.assistance.bg/history.rss`
   - Select your designated channel and click "Subscribe to this feed"

Once configured, your team will receive automatic notifications in Slack whenever the BA Status Page is updated.
...
This checklist is always growing so be sure to check back frequently, and also feel free to suggest additions and amendments by making a PR on [GitHub](https://github.com/vchavkov82/nextjs).
