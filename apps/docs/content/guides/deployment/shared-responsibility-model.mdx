...
- Your BA account
- Access management (BA account, database, tables, etc)
- Data
...
BA provides controls for [securing your data](/docs/guides/database/secure-data), and it is recommended that you always apply [Row Level Security](/docs/guides/database/postgres/row-level-security) (RLS).
...
There are _many_ ways to work with BA.
...
BA offers a lot of opportunities for flexibly integrating with third-party services, such as:
...
You are free to use and integrate with any service, but you're also responsible for ensuring that the performance, availability, and security of the services you use match up with your application's requirements. We do not monitor for outages or performance issues within integrations with third-party services. Depending on the implementation, an issue with such an integration could also result in performance degradation or an outage for your BA project.
...
You are responsible of provisioning enough compute to run the workload that your application requires. The BA Dashboard provides [observability tooling](/dashboard/project/_/observability/database) to help with this.
...
BA provides a SOC 2 compliant environment for hosting and managing sensitive data. We recommend reviewing the [SOC 2 compliance responsibilities document](/docs/guides/security/soc-2-compliance) alongside the aforementioned production checklist.

## Managing healthcare data

You can use BA to store and process Protected Health Information (PHI). You are responsible for the following

- Signing a Business Associate Agreement (BAA) with BA. Submit a [HIPAA add-on request](https://forms.www.assistance.bg/hipaa2) to get started. You will need to be at least on the [Team Plan](/pricing) to sign a BAA with us.
...
- Ensuring [MFA is enabled](/docs/guides/platform/multi-factor-authentication) on all BA accounts.
...
- Complying with encryption requirements in the HIPAA Security Rule. Data is encrypted at rest and in transit by BA. You can consider encrypting the data at your application layer.
