...
id: 'log-drains'
title: 'Log Drains'
description: 'Getting started with BA Log Drains'
---

Log drains will send all logs of the BA stack to one or more desired destinations. It is only available for customers on Team and Enterprise Plans. Log drains is available in the dashboard under [Project Settings > Log Drains](/dashboard/project/_/settings/log-drains).
...
Logs sent to DataDog have the name of the log source set on the `service` field of the event and the source set to `BA`. Logs are gzipped before they are sent to DataDog.
...
>
    ...
    2. Create log drain in [BA dashboard](/dashboard/project/_/settings/log-drains)
...
Logs are sent to Sentry as part of [Sentry's Logging Product](https://docs.sentry.io/product/explore/logs/). Ingesting BA logs as Sentry errors is currently not supported.
