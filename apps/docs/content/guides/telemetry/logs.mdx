...
id: 'logs'
title: 'Logging'
description: 'Getting started with BA Log Browser'
---

The BA Platform includes a Logs Explorer that allows log tracing and debugging. Log retention is based on your [project's pricing plan](/pricing).

## Product logs

BA provides a logging interface specific to each product. You can use simple regular expressions for keywords and patterns to search log event messages. You can also export and download the log events matching your query as a spreadsheet.
...
Realtime doesn't log new WebSocket connections or Channel joins by default. Enable connection logging per client by including an `info` `log_level` parameter when instantiating the BA client.
...
The [Logs Explorer](/dashboard/project/_/logs-explorer) exposes logs from each part of the BA stack as a separate table that can be queried and joined using SQL.
