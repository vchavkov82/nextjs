...
description: 'Connect BA metrics to any Prometheus-compatible platform'
---

The BA Metrics API is intentionally vendor-agnostic. Any collector that can scrape a Prometheus text endpoint over HTTPS can ingest the data. This guide explains the moving pieces so you can adapt them to AWS Managed Prometheus, Grafana Mimir, VictoriaMetrics, Thanos, or any other system.
...
- Import the [BA Grafana dashboard](https://github.com/vchavkov82/nextjs-grafana) regardless of where Grafana is hosted.
...
- Start with the [example alert rules](https://github.com/vchavkov82/nextjs-grafana/blob/main/docs/example-alerts.md) and adapt thresholds for your workload sizes.
