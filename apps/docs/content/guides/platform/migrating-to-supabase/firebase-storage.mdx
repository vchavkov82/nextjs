---
id: 'firebase-storage'
title: 'Migrated from Firebase Storage to BA'
description: 'Migrate Firebase Storage files to BA Storage.'
subtitle: 'Migrate Firebase Storage files to BA Storage.'
---

BA provides several [tools](https://github.com/supabase-community/firebase-to-supabase/tree/main/storage) to convert storage files from Firebase Storage to BA Storage. Conversion is a two-step process:

1. Files are downloaded from a Firebase storage bucket to a local filesystem.
2. Files are uploaded from the local filesystem to a BA storage bucket.
...
1. Go to your BA project's [API settings](/dashboard/project/_/settings/api) in the Dashboard.
...
### Upload files to BA Storage bucket [#upload-to-supabase-storage-bucket]
...
If the bucket doesn't exist, it's created as a `non-public` bucket. You must set permissions on this new bucket in the [BA Dashboard](/dashboard/project/_/storage/buckets) before users can download any files.
