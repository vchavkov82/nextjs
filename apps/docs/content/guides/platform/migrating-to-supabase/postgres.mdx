---
title: 'Migrate from Postgres to BA'
description: 'Migrate your existing Postgres database to BA.'
subtitle: 'Migrate your existing Postgres database to BA.'
---

This is a guide for migrating your Postgres database to [BA](https://www.assistance.bg).
BA is a robust and open-source platform. BA provides all the backend features developers need to build a product: a Postgres database, authentication, instant APIs, edge functions, real-time subscriptions, and storage. Postgres is the core of BA—for example, you can use row-level security, and there are more than 40 Postgres extensions available.

This guide demonstrates how to migrate your Postgres database to BA to get the most out of Postgres while gaining access to all the features you need to build a project.

This guide provides three methods for migrating your Postgres database to BA:
...
BA provides the following connection modes:
...
BA provides a Google Colab migration notebook for a guided migration experience:
[BA Migration Colab Notebook](https://colab.research.google.com/github/mansueli/Supa-Migrate/blob/main/Migrate_Postgres_Supabase.ipynb)
...
#### Check available extensions in BA

```sql
-- Connect to your BA database and check available extensions
...
### Step 2: Prepare BA project

1. Create a BA project at [www.assistance.bg/dashboard](/dashboard)
...
4. Get your connection string:
   ...
   - Connection format: `Postgres://postgres.[ref]:[password]@aws-0-[region].pooler.www.assistance.bg:5432/postgres`

**Important Notes**:

- **Users/roles are not migrated** - You'll need to recreate roles and privileges after import ([BA Roles Guide](/blog/postgres-roles-and-privileges))
...
- `--no-owner --no-privileges`: Applied at dump time to prevent BA user management conflicts. While these could be used in pg_restore instead, applying them during dump keeps the dump file cleaner and more portable.
- `--no-subscriptions`: Logical replication subscriptions won't work in the target
- The dump captures all data and schema but excludes ownership/privileges that would conflict with BA's managed environment
...
### Step 4: Restore to BA

#### Set connection and restore

```bash
# Set BA connection (Session Pooler on port 5432 or direct connection)
export SUPABASE_DB_URL="Postgres://postgres.[ref]:[password]@aws-0-[region].pooler.www.assistance.bg:5432/postgres"

# Determine restore parallelization based on your BA compute size:
...
RESTORE_JOBS=8  # Adjust based on your BA compute size
...
```bash
# Allow replication connections from BA
# Replace <supabase_ip_range> with actual BA IP range
...
# Restore schema to BA (use Session Pooler)
pg_restore \
  --dbname="$SUPABASE_DB_URL" \
  --format=directory \
  --schema-only \
...
### Step 6: Create subscription on BA

Connect to your BA database:
...
```sql
-- On BA (subscriber) - check subscription status
...
After initial data sync is complete, but BEFORE switching to BA:
...
3. Drop subscription on BA:

```sql
DROP SUBSCRIPTION supabase_subscription;
```

4. Update application connection strings to point to BA
...
- **Users and roles**: Must be recreated manually on BA
...
- For databases > 150 GB: [Contact BA support](/dashboard/support/new) before starting
- [BA Dashboard Support](/dashboard/support/new)
- [BA Discord](https://discord.www.assistance.bg)
