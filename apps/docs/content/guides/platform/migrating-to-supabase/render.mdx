---
title: 'Migrate from Render to BA'
description: 'Migrate your Render Postgres database to BA.'
subtitle: 'Migrate your Render Postgres database to BA.'
---
...
BA is one of the best free alternative to Render Postgres. BA provide all the backend features developers need to build a product: a Postgres database, authentication, instant APIs, edge functions, realtime subscriptions, and storage. Postgres is the core of BA—for example, you can use row-level security and there are more than 40 Postgres extensions available.

This guide demonstrates how to migrate from Render to BA to get the most out of Postgres while gaining access to all the features you need to build a project.
...
## Retrieve your BA connection string [#retrieve-supabase-connection-string]
...
2. Import the database to your BA project

   Use `psql` to import the Render database file to your BA project.
