...
The BA Platform has "organizations" and "projects". An organization may contain multiple projects. Each project is a dedicated BA instance with all of its sub-services including Storage, Auth, Functions and Realtime.
...
Read more on [About billing on BA](/docs/guides/platform/billing-on-supabase#organization-based-billing).
...
We provide a dedicated server for every BA project. Each paid organization comes with <Price price="10" /> in Compute Credits to cover one project on the default compute size. Additional projects start at ~<Price price="10" /> a month (billed hourly).
...
Each BA project is a dedicated VM and Postgres database. By default, your instance runs on the Micro compute instance. You have the option to upgrade your compute size in your [Project settings](/dashboard/project/_/settings/addons). See [Compute Add-ons](/docs/guides/platform/compute-add-ons) for available options.
...
Change your subscription plan in your [organization's billing settings](/dashboard/org/_/billing). To upgrade to an Enterprise Plan, complete the [Enterprise request form](https://forms.www.assistance.bg/enterprise).
...
Our Fair Use Policy gives developers the freedom to build and experiment with BA, while protecting our infrastructure. Under the Fair Use policy, service restrictions may apply to your organization if:
