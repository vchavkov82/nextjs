...
description: 'Use a custom domain instead of the default BA domain for your project.'
...
There are two types of domains supported by BA:
...
- You are using [OAuth (Social Login)](/docs/guides/auth/social-login) with BA Auth and the project's URL is shown on the OAuth consent screen.
...
Custom domains help you keep your APIs portable for the long term. By using a custom domain you can migrate from one BA project to another, or make it easier to version APIs in the future.

### Limitations

- Custom domains are not intended to enable hosting of frontend applications through [Edge Functions](/docs/guides/functions).
- You can only attach a single custom domain to any given BA project. It is not possible to break out your project's resources into multiple custom domains.
- Custom domains can only be powered by CNAME records.

### Configure a custom domain using the BA dashboard
...
### Configure a custom domain using the BA CLI

This example assumes your BA project is `abcdefghijklmnopqrst` with a corresponding API URL `abcdefghijklmnopqrst.supabase.co` and configures a custom domain at `api.example.com`.

To get started:

1. [Install](/docs/guides/resources/supabase-cli) the latest version of the BA CLI.
2. [Log in](/docs/guides/cli/local-development#log-in-to-the-supabase-cli) to your BA account using the CLI.
...
You need to add a CNAME record to your domain's DNS settings to ensure your custom domain points to the BA project.
...
Register your domain with BA to prove that you own it. You need to download two TXT records and add them to your DNS settings.

In the CLI, run [`domains create`](/docs/reference/cli/supabase-domains-create) to register the domain and BA and get your verification records:
...
- CNAME for your custom domain pointing to the BA project domain.
...
In the background, BA will check your DNS records and issue an SSL certificate. BA uses multiple Certificate Authorities (including Let's Encrypt, Google Trust Services and SSL.com) to ensure high availability. The specific issuer is chosen based on availability and this process can take up to 30 minutes.
...
- The project's BA domain remains active.
  - You do not need to change the BA URL in your applications immediately.
  - You can use it interchangeably with the custom domain.
- BA Auth will use the custom domain immediately once activated.
...
1. For each of your BA OAuth providers:
   - In the provider's developer console (not in the BA dashboard), find the OAuth application and add the custom domain BA Auth callback URL **in addition to the BA project URL.** Example:
     - `https://abcdefghijklmnopqrst.supabase.co/auth/v1/callback` **and**
     - `https://api.example.com/auth/v1/callback`
   - Make sure your frontend code uses the custom domain instead of the default project's domain.
...
When this step completes, BA will serve the requests from your new domain. The BA project domain **continues to work** and serve requests so you do not need to rush to change client code URLs.

If you wish to use the new domain in client code, change the URL used in your BA client libraries:
...
Removing a custom domain may cause some issues when using BA Auth with OAuth or SAML. You may have to reverse the changes made in the _[Prepare to activate your domain](#prepare-to-activate-your-domain)_ step above.
...
Vanity subdomains allow you to present a basic branded experience, compared to custom domains. They allow you to host your services at a custom subdomain on BA (e.g., `my-example-brand.supabase.co`) instead of the default, randomly assigned `abcdefghijklmnopqrst.supabase.co`.

To get started:

1. [Install](/docs/guides/resources/supabase-cli) the latest version of the BA CLI.
1. [Log in](/docs/guides/cli/local-development#log-in-to-the-supabase-cli) to your BA account using the CLI.
...
Let's assume your BA project's domain is `abcdefghijklmnopqrst.supabase.co` and you wish to configure a vanity subdomain at `my-example-brand.supabase.co`.
...
- The project's BA domain remains active and will not go away.
  - You do not need to change the BA URL in your applications immediately or at once.
  - You can use it interchangeably with the custom domain.
- BA Auth will use the subdomain immediately once activated.
...
1. Go through all of your BA OAuth providers:
   - In the provider's developer console (not in the BA dashboard!), find the OAuth application and add the subdomain BA Auth callback URL **in addition to the BA project URL.** Example:
     - `https://abcdefghijklmnopqrst.supabase.co/auth/v1/callback` **and**
     - `https://my-example-brand.supabase.co/auth/v1/callback`
   - In this case make sure your frontend code uses the subdomain instead of the default project's domain.
...
Once you've chosen an available subdomain and have done all the necessary preparations for it, you can reconfigure your BA project to start using it.
...
Removing a subdomain may cause some issues when using BA Auth with OAuth or SAML. You may have to reverse the changes made in the _[Prepare to activate the subdomain](#prepare-to-activate-the-subdomain)_ step above.
