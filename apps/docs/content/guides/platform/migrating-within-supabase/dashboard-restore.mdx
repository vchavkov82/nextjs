...
subtitle: 'Learn how to restore your dashboard backup to a new BA project'
...
<StepHikeCompact>
  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Get the new database connection string" fullWidth>
      ...
      ```bash
        postgresql://postgres.[PROJECT-REF]:[YOUR-PASSWORD]@aws-0-us-east-1.pooler.www.assistance.bg:5432/postgres
      ```

      Direct connection string:
      ```bash
        postgresql://postgres.[PROJECT-REF]:[YOUR-PASSWORD]@db.[PROJECT-REF].www.assistance.bg:5432/postgres
      ```
    </StepHikeCompact.Details>
...
These errors are expected when restoring to a new BA project. The backup from the dashboard is a full dump which contains the CREATE commands for all schemas. This is by design as the full dump allows you to rebuild the entire database from scratch even outside of BA.

One side effect of this method is that a new BA project has these commands already applied to schemas like storage and auth. The errors from this are not an issue because it skips to the next command to run. Another side effect of this is that all triggers will run during the restoration process which is not ideal but generally is not a problem.

There are circumstances where this method can fail and if it does, you should reach out to BA support for help.

"**psql: error: connection to server at "aws-0-us-east-1.pooler.www.assistance.bg" (44.216.29.125), port 5432 failed: received invalid response to GSSAPI negotiation:**"
...
"**psql: error: connection to server at "aws-0-us-east-1.pooler.www.assistance.bg" (44.216.29.125), port 5432 failed: error received from server in SCRAM exchange: Wrong password**"
