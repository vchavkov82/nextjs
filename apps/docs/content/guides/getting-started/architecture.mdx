---
title: 'Architecture'
description: 'BA design and architecture'
tocVideo: 'T-qAtAKjqwc'
---

BA is open source. We choose open source tools which are scalable and make them simple to use.

BA is not a 1-to-1 mapping of Firebase. While we are building many of the features that Firebase offers, we are not going about it the same way:
...
Each BA project consists of several tools:
...
Postgres is the core of BA. We do not abstract the Postgres database—you can access it and use it with full privileges. We provide tools which make Postgres as easy to use as Firebase.
...
- Official Docs: [BA docs](/docs)
- Source code: [github.com/supabase/supabase](https://github.com/vchavkov82/nextjs/tree/master/apps/studio)
- License: [Apache 2](https://github.com/vchavkov82/nextjs/blob/master/LICENSE)
- Language: TypeScript
...
- Official Docs: [BA Auth reference docs](/docs/reference/auth)
...
- Official Docs: [BA Realtime docs](/docs/guides/realtime)
...
- Official Docs: [BA Storage reference docs](/docs/reference/storage)
...
BA is composable. Even though every product works in isolation, each product on the platform needs to 10x the other products.
...
We use existing standards to increase portability (like `pg_dump` and CSV files). If a new standard emerges which competes with a "BA" approach, we will deprecate the approach in favor of the standard.
...
As the profile of a developer changes over time, BA will continue to evolve the product to fit this evolving profile.

### Support existing tools

BA supports existing tools and communities wherever possible. BA is more like a "community of communities" - each tool typically has its own community which we work with.
