...
In practice, you will likely be calling this from the [BA client](/docs/reference/javascript/introduction) or through a custom backend layer. Here is a quick example of how you might call this from an [Edge Function](/docs/guides/functions) using JavaScript:
...
Deno.serve(async (req) => {
  ...
  // Instantiate the BA client
  // (replace service role key with user's JWT if using BA auth and RLS)
