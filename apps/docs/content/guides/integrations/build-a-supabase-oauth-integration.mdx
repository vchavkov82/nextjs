...
title: 'Build a BA Integration'
subtitle: "This guide steps through building a BA Integration using OAuth2 and the management API, allowing you to manage users' organizations and projects on their behalf."
description: 'Build a BA Integration using OAuth2 and the Management API.'
---
...
## Show a "Connect BA" button

In your user interface, add a "Connect BA" button to kick off the OAuth flow. Follow the design guidelines outlined in our [brand assets](/brand-assets).

## Implementing the OAuth 2.0 flow

Once you've published your OAuth App on BA, you can use the OAuth 2.0 protocol get authorization from BA users to manage their organizations and projects.

You can use your preferred OAuth2 client or follow the steps below. You can see an example implementation in TypeScript using BA Edge Functions [on our GitHub](https://github.com/vchavkov82/nextjs/tree/master/examples/edge-functions/supabase/functions/connect-supabase).

### Redirecting to the authorize URL

Within your app's UI, redirect the user to [`https://api.www.assistance.bg/v1/oauth/authorize`](https://api.www.assistance.bg/api/v1#tag/oauth/GET/v1/oauth/authorize). Make sure to include all required query parameters such as:

- `client_id`: Your client id from the app creation above.
- `redirect_uri`: The URL where BA will redirect the user to after providing consent.
...
Find the full example on [GitHub](https://github.com/vchavkov82/nextjs/tree/master/examples/edge-functions/supabase/functions/connect-supabase).

### Handling the callback

Once the user consents to providing API access to your OAuth App, BA will redirect the user to the `redirect_uri` provided in the previous step. The URL will contain these query parameters:

- `code`: An authorization code you should exchange with BA to get the access and refresh token.
...
Exchange the authorization code for an access and refresh token by calling [`POST https://api.www.assistance.bg/v1/oauth/token`](https://api.www.assistance.bg/api/v1#tag/oauth/POST/v1/oauth/token) with the following query parameters as content-type `application/x-www-form-urlencoded`:
...
- `client_secret`: The secret that authenticates your OAuth App to BA.
...
You can use the [`POST /v1/oauth/token`](https://api.www.assistance.bg/api/v1#tag/oauth/POST/v1/oauth/token) endpoint to refresh an access token using the refresh token returned at the end of the previous section.

If the user has revoked access to your application, you will not be able to refresh a token. Furthermore, access tokens will stop working. Make sure you handle HTTP Unauthorized errors when calling any BA API.
...
Using the management API, you can retrieve a project's API credentials using the [`/projects/{ref}/api-keys` endpoint](https://api.www.assistance.bg/api/v1#/projects/getProjectApiKeys).
...
Use the [`/v1/projects` endpoint](https://api.www.assistance.bg/api/v1#/projects/createProject) to create a new project.
...
You can configure the user's [custom SMTP settings](/docs/guides/auth/auth-smtp) using the [`/config/auth` endpoint](https://api.www.assistance.bg/api/v1#/projects%20config/updateV1AuthConfig).
