...
subtitle: 'Learn about authenticating with BA Storage S3.'
sidebar_label: 'S3'
---

You have two options to authenticate with BA Storage S3:
...
This is all the information you need to connect to BA Storage using any S3-compatible service.
...
You can authenticate to BA S3 with a user JWT token to provide limited access via RLS to all S3 operations. This is useful when you want initialize the S3 client on the server scoped to a specific user, or use the S3 client directly from the client side.
