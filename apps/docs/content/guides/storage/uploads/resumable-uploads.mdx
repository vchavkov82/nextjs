...
description: 'Learn how to upload files to BA Storage.'
subtitle: 'Learn how to upload files to BA Storage.'
...
>
    ...
    async function uploadFile(bucketName, fileName, file) {
        ...
        return new Promise((resolve, reject) => {
            var upload = new tus.Upload(file, {
                // BA TUS endpoint (with direct storage hostname)
  ...
  <TabPanel id="react" label="React">
    ...
    /**
     * Custom hook for configuring Uppy with BA authentication and TUS resumable uploads
     * @param {Object} options - Configuration options for the Uppy instance.
     * @param {string} options.bucketName - The bucket name in BA where files are stored.
    ...
    export const useUppyWithSupabase = ({ bucketName }: { bucketName: string }) => {
        ...
        // Initialize BA client with project URL and anon key
        ...
        useEffect(() => {
            ...
            // Initialize Uppy with BA settings
            initializeUppy();
        }, [uppy, bucketName]);
...
You can check a [full example using Uppy](https://github.com/vchavkov82/nextjs/tree/master/examples/storage/resumable-upload-uppy).
...
See this [full example using Uppy with signed URLs](https://github.com/vchavkov82/nextjs/tree/master/examples/storage/resumable-upload-signed-uppy) for more context.
