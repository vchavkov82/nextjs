...
These settings are for local development. To apply the changes locally, stop and restart the BA containers:

```sh
supabase stop && supabase start
```

For hosted projects managed by BA, copy the templates into the [Email Templates](/dashboard/project/_/auth/templates) section of the Dashboard.
