---
title: 'BA CLI'
description: 'The BA CLI provides tools to develop your project locally, deploy to the BA Platform, and set up CI/CD workflows.'
subtitle: 'Develop locally, deploy to the BA Platform, and set up CI/CD workflows'
---

The BA CLI enables you to run the entire BA stack locally, on your machine or in a CI environment. With just two commands, you can set up and start a new local project:

1. `supabase init` to create a new local project
2. `supabase start` to launch the BA services

## Installing the BA CLI
...
The BA CLI requires **Node.js 20 or later** when run via `npx` or `npm`. Older Node.js versions, such as 16, are not supported and fail to start the CLI.
...
1. Download the latest package from the [BA CLI releases page](https://github.com/supabase/cli/releases/latest)
...
If you have any BA containers running locally, stop them and delete their data volumes before proceeding with the upgrade. This ensures that BA managed services can apply new migrations on a clean state of the local database.
...
The BA CLI uses Docker containers to manage the local development stack. Follow the official guide to install and configure [Docker Desktop](https://docs.docker.com/desktop):
...
Now, to start the BA stack, run:

```bash
supabase start
```

This takes time on your first run because the CLI needs to download the Docker images to your local machine. The CLI includes the entire BA toolset, and a few additional images that are useful for local development (like a local SMTP server and a database diff tool).

## Access your project's services

Once all of the BA services are running, you'll see output containing your local BA credentials. It should look like this, with urls and keys that you'll use in your local project:
...
The local development environment includes BA Studio, a graphical interface for working with your database.
...
To access the database from an edge function in your local BA setup, replace `localhost` with `host.docker.internal`.
...
When you are finished working on your BA project, you can stop the stack (without resetting your local database):
