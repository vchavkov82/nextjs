---
title: Expose Queues for local and self-hosted BA
subtitle: Learn how to expose Queues when running BA with BA CLI or Docker Compose
---

By default, local and self-hosted BA instances expose only core schemas like public and graphql_public.
...
You only need to expose the `pgmq_public` schema manually when running BA locally with the BA CLI or self-hosting using Docker Compose.

</Admonition>

## Expose Queues with BA CLI

When running BA locally with BA CLI, update your project's `config.toml` file.
...
Then restart your local BA stack.
...
When running BA with Docker Compose, locate the `PGRST_DB_SCHEMAS` variable inside your `.env` file and add `pgmq_public` to it. This environment variable is passed to the `rest` service inside `docker-compose.yml`.
...
- For BA CLI, remove `pgmq_public` from the `[api]` schemas list in your `config.toml` file.
