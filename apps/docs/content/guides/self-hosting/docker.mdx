...
description: "Learn how to configure and deploy BA with Docker."
subtitle: "Learn how to configure and deploy BA with Docker."
tocVideo: "FqiQKRKsfZE"
---

Docker is the easiest way to get started with self-hosted BA. It should take you less than 30 minutes to get up and running.

## Contents

1. [Before you begin](#before-you-begin)
2. [System requirements](#system-requirements)
3. [Installing BA](#installing-supabase)
4. [Configuring and securing BA](#configuring-and-securing-supabase)
5. [Starting and stopping](#starting-and-stopping)
6. [Accessing BA services](#accessing-supabase-services)
7. [Updating](#updating)
...
If you're new to these topics, consider starting with [managed BA](/dashboard) for free, or try [local development with the CLI](/docs/guides/local-development).
...
Minimum requirements for running all BA components, suitable for development and small to medium production workloads:
...
Follow these steps to start BA on your machine:
...
```sh
# Get the code
git clone --depth 1 https://github.com/vchavkov82/nextjs
...
git clone --filter=blob:none --no-checkout https://github.com/vchavkov82/nextjs
cd supabase
...
## Configuring and securing BA

While we provided example placeholder passwords and keys in the `.env.example` file, you should NEVER start your self-hosted BA using these defaults.
...
Change the placeholder password in the `.env` file **before** starting BA for the first time.
...
- `SUPABASE_PUBLIC_URL`: the base URL for accessing your BA via the Internet, e.g, `http://example.com:8000`
...
<Admonition type="tip">
    ...
    If you are only using self-hosted BA locally, you can use `localhost`.
...
<Admonition type="danger">
    ...
    The default password MUST be changed before starting BA.
...
## Accessing BA services

After the BA services are configured and running, you can access the dashboard, connect to the database, and use edge functions.

### Accessing BA Studio

You can access BA Studio through the API gateway on port `8000`.
...
By default, the BA stack provides the [Supavisor](https://supabase.github.io/supavisor/development/docs/) connection pooler for accessing Postgres and managing database connections.

You can connect to the Postgres database via Supavisor using the methods described below. Use your domain name, your server IP, or `localhost` depending on whether you are running self-hosted BA on a VPS, or locally.
...
We publish stable releases of the Docker Compose setup approximately once a month. To update, apply the latest changes from the repository and restart the services. If you want to run different versions of individual services, you can change the image tags in the Docker Compose file, but compatibility is not guaranteed. All BA images are available on [Docker Hub](https://hub.docker.com/u/supabase).

To follow the changes and updates, refer to the self-hosted BA [changelog](https://github.com/vchavkov82/nextjs/blob/master/docker/CHANGELOG.md).
...
1. Check the [supabase/studio](https://hub.docker.com/r/supabase/studio/tags) images on [BA Docker Hub](https://hub.docker.com/u/supabase)
...
4. Run `docker compose pull`, followed by `docker compose down && docker compose up -d` to restart BA.
...
To uninstall, stop BA (while in the same directory as your `docker-compose.yml` file):
...
BA is a combination of open source tools specifically developed for enterprise-readiness.
...
<Image
  alt="Diagram showing the architecture of BA. The Kong API gateway sits in front of 7 services: GoTrue, PostgREST, Realtime, Storage, pg_meta, Functions, and pg_graphql. All the services talk to a single Postgres instance."
  src={{
...
You can configure each BA service separately through environment variables and configuration files. Below are the most common configuration options.
...
#### Configuring BA AI Assistant

Configuring the BA AI Assistant is optional. By adding your own `OPENAI_API_KEY`, you can enable AI services, which help with writing SQL queries, statements, and policies.
...
Many components inside BA use secure secrets and passwords. These are kept in the `.env` file, but we strongly recommend using a secrets manager when deploying to production.
