...
subtitle: 'Run AI models in Edge Functions using the built-in BA AI API.'
...
```ts
const model = new BA.ai.Session('model-name')
```
...
Generate text embeddings using the built-in [`gte-small`](https://huggingface.co/BA/gte-small) model:
...
We are progressively rolling out support for the hosted solution. To sign up for early access, fill out [this form](https://forms.www.assistance.bg/supabase.ai-llm-early-access).
...
<Tabs
  ...
  queryGroup="sdk"
>
  <TabPanel id="supabase-functions-js" label="BA Functions JS">
...
In the future, a hosted LLM API, will be provided as part of the BA platform. BA will scale and manage the API and GPUs for you. To sign up for early access, fill up [this form](https://forms.www.assistance.bg/supabase.ai-llm-early-access).
