...
The hosted BA Platform supports the [`pg_cron` extension](/docs/guides/database/extensions/pgcron), a recurring job scheduler in Postgres.
...
To access the auth token securely for your Edge Function call, we recommend storing them in [BA Vault](/docs/guides/database/vault).
...
Store `project_url` and `anon_key` in BA Vault:
...
Make a POST request to a BA Edge Function every minute:
