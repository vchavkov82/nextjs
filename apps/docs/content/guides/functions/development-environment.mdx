...
The BA CLI doesn't use the standard Deno CLI to serve functions locally. Instead, it uses its own Edge Runtime to keep the development and production environment consistent.
...
You can find the complete example on [GitHub](https://github.com/vchavkov82/nextjs/tree/master/examples/edge-functions).
...
└── supabase
    ...
    │   │   ├── supabaseAdmin.ts # BA client with SERVICE_ROLE key
    │   │   ├── supabaseClient.ts # BA client with ANON key
...
This command spins up your entire BA stack locally: database, auth, storage, and Edge Functions runtime. You're developing against the exact same environment you'll deploy to.
