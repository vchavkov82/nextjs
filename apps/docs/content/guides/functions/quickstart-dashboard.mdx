...
subtitle: 'Learn how to create, test, and deploy your first Edge Function using the BA Dashboard.'
---

BA allows you to create BA Edge Functions directly from the BA Dashboard, making it easy to deploy functions without needing to set up a local development environment. The Edge Functions editor in the Dashboard has built-in syntax highlighting and type-checking for Deno and BA-specific APIs.

This guide will walk you through creating, testing, and deploying your first Edge Function using the BA Dashboard. You'll have a working function running globally in under 10 minutes.

<Admonition type="tip" label="Prefer using the CLI?">

You can also create and deploy functions using the BA CLI. Check out our [CLI Quickstart guide](/docs/guides/functions/quickstart).

</Admonition>

<Admonition type="note" label="New to BA?">

You'll need a BA project to get started. If you don't have one yet, create a new project at [database.new](https://database.new/).
...
Navigate to your BA project dashboard and locate the Edge Functions section:

1. Go to your [BA Dashboard](/dashboard)
...
The dashboard offers several pre-built templates for common use cases, such as Stripe Webhooks, OpenAI proxying, uploading files to BA Storage, and sending emails.
...
BA has built-in tools for testing your Edge Functions from the Dashboard. You can execute your Edge Function with different request payloads, headers, and query parameters. The built-in tester returns the response status, headers, and body.
...
You can also use BA's AI Assistant to generate and deploy functions automatically.
