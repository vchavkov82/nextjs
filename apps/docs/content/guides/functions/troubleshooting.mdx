...
Before troubleshooting, make sure you're using the latest version of the BA CLI:
...
If these steps don't resolve the issue, open a support ticket via the BA Dashboard and
...
- **Event Loop Completion:** If logs show "Event loop completed," your function has implementation issues. You should check your function code for any syntax errors, infinite loops, or unresolved promises that might cause this error.
  ...
  You can also try running the function locally (using BA CLI **`functions serve`**) to see if you can debug the error. The local console should give a full stack trace on the error with line numbers of the source code. You can also refer to [Edge Functions examples](https://github.com/vchavkov82/nextjs/tree/master/examples/edge-functions) for guidance.
...
ticket via the BA Dashboard (by clicking the "Help" button at the top right) and include all
