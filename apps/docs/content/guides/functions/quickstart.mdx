...
description: 'Get started with BA Edge Functions.'
subtitle: 'Learn how to create, test, and deploy your first Edge Function using the BA CLI.'
---

Before getting started, make sure you have the **BA CLI installed**. Check out the [CLI installation guide](/docs/guides/cli) for installation methods and troubleshooting.

<Admonition type="tip" label="Prefer using the BA Dashboard?">

You can also create and deploy functions directly from the BA Dashboard. Check out our [Dashboard Quickstart guide](/docs/guides/functions/quickstart-dashboard).
...
If you don't have a project yet, initialize a new BA project in your current directory.
...
Or, if you already have a project locally, navigate to your project directory. If your project hasn't been configured for BA yet, make sure to run the `supabase init` command.

```bash
cd your-existing-project
supabase init # Initialize BA, if you haven't already
```
...
```bash
supabase start  # Start all BA services
supabase functions serve hello-world
```

<Admonition type="note" label="First time running BA services?">
...
- Stop other BA instances with `supabase stop`
...
After this step, you should have all BA services running locally, and your Edge Function serving at the local URL. Keep these terminal windows open.
...
## Step 5: Connect to your BA project

To deploy your function globally, you need to connect your local project to a BA project.

<Admonition type="tip" label="Need to create new BA project?">

Create one at [database.new](https://database.new/).

</Admonition>

First, login to the CLI if you haven't already, and authenticate with BA. This opens your browser to authenticate with BA; complete the login process in your browser.

```bash
supabase login
```

Next, list your BA projects to find your project ID:

```bash
supabase projects list
```

Next, copy your project ID from the output, then connect your local project to your remote BA project. Replace `YOUR_PROJECT_ID` with the ID from the previous step.
...
After this step, you should have your local project authenticated and linked to your remote BA project. You can verify this by running `supabase status`.
...
Deploy your function to BA's global edge network:
...
The `SUPABASE_PUBLISHABLE_KEY` is different in development and production. To get your production anon key, you can find it in your BA dashboard under **Settings > API**.
...
<TabPanel id="supabase-js" label="BA Client">
