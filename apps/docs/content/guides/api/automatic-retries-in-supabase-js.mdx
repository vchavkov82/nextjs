...
subtitle: 'Learn how to add automatic retries to your BA API requests using `fetch-retry`.'
---
...
The `fetch-retry` package allows you to add retry logic to `fetch` requests, making it a useful tool for enhancing the resilience of API calls in your BA applications. Here's a step-by-step guide on how to integrate `fetch-retry` with the `supabase-js` library.
...
// Create a BA client instance with the custom fetch
...
## 4. Using the BA client

With `fetch-retry` integrated, you can use the BA client as usual. The retry logic will automatically apply to all network requests made by `supabase-js`.
...
Integrating `fetch-retry` with `supabase-js` is a straightforward way to add robustness to your BA API requests. By handling transient errors and implementing retry strategies, you can improve the reliability of your application while maintaining a seamless user experience.
