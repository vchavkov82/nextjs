...
This guide addresses common Prisma errors that you might encounter while using BA.
...
- **BA Managed Schemas**: BA may update managed schemas like auth and storage to introduce new features. Granting Prisma access to these schemas can lead to drift during updates.
...
- **Transaction Mode for serverless apps**: If you are using serverless functions (BA Edge, Vercel, AWS Lambda), switch to transaction mode (port 6543). It handles more connections than session mode or direct connections.
...
- Multi-Schema support: If the external schema isn't BA managed, modify your `prisma.schema` file to enable the multi-Schema preview
...
- BA managed schemas: Schemas managed by BA, such as `auth` and `storage`, may be changed to support new features. Referencing these schemas directly will cause schema drift in the future. It is best to remove references to these schemas from your migrations.
