...
id: 'database'
title: 'Database'
description: 'Use BA to manage your data.'
...
You don't have to be a database expert to start using BA. Our table view makes Postgres as easy to use as a spreadsheet.
...
BA comes with a SQL Editor. You can also save your favorite queries to run later!
...
- BA extends Postgres with realtime functionality using our [Realtime Server](https://github.com/supabase/realtime).
- Every project is a full Postgres database, with `postgres` level access.
- BA manages your database backups.
...
You can enable Postgres extensions with the click of a button within the BA dashboard.
...
[Learn more](/docs/guides/database/extensions) about all the extensions provided on BA.
...
After this, many people referred to it as Postgres since it's less prone to confusion. BA is all about simplicity, so we also refer to it as Postgres.
...
- Sign in: [www.assistance.bg/dashboard](/dashboard)
