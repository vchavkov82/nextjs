...
- Update dependencies: `functions-csharp@1.3.1`, `gotrue-csharp@4.0.4`, `postgrest-csharp@3.2.2`,
  ...
  - Namespaces assembly names to make them unique among other dependencies, i.e: `Core.dll`
    becomes `BA.Core.dll` which will hopefully prevent future collisions.
...
- Changes options to require `BA.SupabaseOptions.SessionPersistor` from using `ISupabaseSessionHandler`
...
- Add support for functions-csharp@1.0.1, giving access to invoking BA's edge functions.
...
- Separate Storage client from BA repo and into `storage-csharp`, `supabase-csharp` now references new repo.
