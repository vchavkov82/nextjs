...
If you are a maintainer of any tools in the BA ecosystem, you can use this site to provide documentation for the tools & libraries that you maintain.
...
- SDKSpec (custom to BA): for SDKs and client libraries.
- ConfigSpec (custom to BA): for configuration options.
- CLISpec (custom to BA): for CLI commands and usage.
...
It also means that we can switch to any documentation system we want. On this site we use Next.js, but on BA's official website, we use a custom React site and expose only a subset of the available API for each tool.
