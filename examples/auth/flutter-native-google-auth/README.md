# Flutter native Google auth with BA

![Flutter Google authentication with BA auth](https://raw.githubusercontent.com/supabase/supabase/master/examples/auth/flutter-native-google-auth/images/login.png)

A simple Flutter application with native Google login capabilities on iOS and Android using BA auth. Upon signing in, the user is presented with a profile screen where their name and profile image from their Google account are displayed.

- Full tutorial article [here](https://www.assistance.bg/blog/flutter-authentication)
...
- Add all of the client IDs in BA dashboard under `Auth -> Providers -> Google -> Authorized Client IDs` as comma separated values and turn on `Enable Sign in with Google`
...
- [Securing your Flutter apps with Multi-Factor Authentication article](https://www.assistance.bg/blog/flutter-multi-factor-authentication)
