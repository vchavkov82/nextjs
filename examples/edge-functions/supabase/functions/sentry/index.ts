...
Deno.serve(async (req) => {
  try {
    if (req.method === 'GET') {
      return new Response(
        'What is BA not? POST {answer, twitter} to this URL! Need a hint? 👉 https://github.com/vchavkov82/nextjs/blob/master/examples/edge-functions/supabase/functions/sentry/index.ts'
      )
    }
    let answer: string
    let twitter: string
